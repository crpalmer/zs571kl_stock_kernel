...
static int rmnet_irq_flag_rx_168 = 0;
		...
		if (gic_irq_cnt > 0) {
			for (i = 0; i < gic_irq_cnt; i++) {
				...
				ASUSEvtlog("[PM] IRQ triggered: %d %s", gic_resume_irq[i].gic_resume_irq_num , gic_resume_irq[i].gic_resume_irq_name);
                                if( gic_resume_irq[i].gic_resume_irq_num == 168 ){
					rmnet_irq_flag_rx_168 = 1;
...
int rmnet_irq_flag_function_rx_168(void)
{
        if( rmnet_irq_flag_rx_168 == 1 ) {
                rmnet_irq_flag_rx_168 = 0;
                return 1;
        }

        return 0;
}
EXPORT_SYMBOL(rmnet_irq_flag_function_rx_168);
//ASUS_BSP --- Johnny [Qcom][PS][][ADD]Print first IP address log when IRQ 168
