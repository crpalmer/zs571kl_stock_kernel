...
void asus_get_disk_stat(void)
{
	...
	r_tick = w_tick = io_tick = in_flight = 0;
		...
		while ((part = disk_part_iter_next(&piter))) {
			if((!(strcmp("dm-0",disk_name(disk, part->partno, buf))) || !(strcmp("dm-1",disk_name(disk, part->partno, buf))) 
				|| !(strcmp("sde31",disk_name(disk, part->partno, buf))))){
				if(g_is_lock && !(strcmp("sde31",disk_name(disk, part->partno, buf))))
					continue;
				cpu = part_stat_lock();
				...
				if (g_is_lock){
					if(!strcmp("dm-1", disk_name(disk,part->partno,buf))){
