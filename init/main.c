...
int g_is_lock = 0;
EXPORT_SYMBOL(g_is_lock);
static int get_lock_stat(char *str)
{
    if( !strcmp("Y", str))
	g_is_lock = 0;
    else
...
__setup("UNLOCKED=", get_lock_stat);
...
enum DEVICE_RFSKU g_ASUS_rfSKU = RF_SKU_INVALID;
enum DEVICE_DISPLAYID g_ASUS_displayID = DISPLAY_VENDOR_INVALID;
enum DEVICE_DEPTHID g_ASUS_depthID = DEPTH_VENDOR_INVALID;
enum DEVICE_FPID g_ASUS_fpID = FP_VENDOR_INVALID;
enum DEVICE_EVBCFGID g_ASUS_evbcfgID = EVBCFG_ID_INVALID;

EXPORT_SYMBOL(g_ASUS_hwID);
EXPORT_SYMBOL(g_ASUS_projID);
EXPORT_SYMBOL(g_ASUS_rfSKU);
EXPORT_SYMBOL(g_ASUS_displayID);
EXPORT_SYMBOL(g_ASUS_depthID);
EXPORT_SYMBOL(g_ASUS_fpID);
EXPORT_SYMBOL(g_ASUS_evbcfgID);
#endif
...
static int of_get_pcbid_data(void)
{
       ...
       pcbid_str = of_get_property(np, "rf_sku", NULL);
       if (pcbid_str == NULL)
               pr_err("Fail to get RF SKU though dt\n");
       else if (!kstrtoint(pcbid_str, 0, &value)) {
               g_ASUS_rfSKU = value;
               pr_info("PCBID: Get RF SKU: %d\n", g_ASUS_rfSKU);
       }

       pcbid_str = of_get_property(np, "display_id", NULL);
       if (pcbid_str == NULL)
               pr_err("Fail to get DISPLAY ID though dt\n");
       else if (!kstrtoint(pcbid_str, 0, &value)) {
               g_ASUS_displayID = value;
               pr_info("PCBID: Get DISPLAY ID: %d\n", g_ASUS_displayID);
       }

       pcbid_str = of_get_property(np, "depth_id", NULL);
       if (pcbid_str == NULL)
               pr_err("Fail to get DEPTH ID though dt\n");
       else if (!kstrtoint(pcbid_str, 0, &value)) {
               g_ASUS_depthID = value;
               pr_info("PCBID: Get DEPTH ID: %d\n", g_ASUS_depthID);
       }

       pcbid_str = of_get_property(np, "fp_id", NULL);
       if (pcbid_str == NULL)
               pr_err("Fail to get FP ID though dt\n");
       else if (!kstrtoint(pcbid_str, 0, &value)) {
               g_ASUS_fpID = value;
               pr_info("PCBID: Get FP ID: %d\n", g_ASUS_fpID);
       }

       pcbid_str = of_get_property(np, "evbcfg_id", NULL);
       if (pcbid_str == NULL)
               pr_err("Fail to get EVBCFG ID though dt\n");
       else if (!kstrtoint(pcbid_str, 0, &value)) {
               g_ASUS_evbcfgID = value;
               pr_info("PCBID: Get EVBCFG ID: %d\n", g_ASUS_evbcfgID);
       }
