 ...
 */
static char *fw_prefix = "../stereo_right.cnt";
module_param(fw_prefix, charp, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(fw_prefix, "TFA98xx DSP firmware (container file) name.");
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	if (!tfa_cnt_get_app_name(tfa98xx->fw.name))
		strcpy(tfa98xx->fw.name, "tfa98xx");
	...
	pr_debug("Firmware init complete (%s)\n", tfa98xx->fw.name);
...
static int tfa98xx_get_fw_by_revision(int rev, const char *prefix, char *name)
{
	char *last_dot;
...
static int tfa98xx_load_container(struct tfa98xx *tfa98xx)
{
	...
	tfa98xx->fw_name =
		devm_kzalloc(tfa98xx->codec->dev, 256, GFP_KERNEL);

	if (tfa98xx_get_fw_by_revision(reg & 0xff, fw_prefix, tfa98xx->fw_name) != 0)
		pr_warn("Unable determine firmware to load, using %s\n",
				tfa98xx->fw_name);
	pr_info("%s: Loading %s\n", __func__, tfa98xx->fw_name);
						...
						struct snd_soc_dai *dai)
{
	...
	/* By default the sample rate was 48000, let's hardcode the
	 * supported sample rates. */
	tfa98xx->rate_constraint.count =
		ARRAY_SIZE(supported_sample_rates);
	memcpy(tfa98xx->rate_constraint_list,
