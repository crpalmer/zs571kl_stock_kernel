...
/*
 * lcd_err_handler() - Interrupt handler for LCD error signal.
 * @irq		: irq line number
 * @data	: Pointer to the device structure.
 *
 * This function is called whenever a LCD error signal is received from the
 * panel. This reboots the panel.
 */
irqreturn_t lcd_err_handler(int irq, void *data)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata =
			(struct mdss_dsi_ctrl_pdata *)data;
	...
	queue_delayed_work(ctrl_pdata->lcd_err_workq,
				&ctrl_pdata->lcd_err_work, 0);
