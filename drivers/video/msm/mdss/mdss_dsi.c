...
struct mdss_dsi_ctrl_pdata *g_ctrl_pdata_0;
...
int hdmi_tx_sysfs_enable_hpd(struct hdmi_tx_ctrl *hdmi_ctrl, int on);
extern struct hdmi_tx_ctrl *g_hdmi_ctrl;

extern int last_bk_level;
...
static int mdss_dsi_panel_power_lp(struct mdss_panel_data *pdata, int enable)
{
	...
	if (!enable) {
		pr_err("[DISP] Disable Always on\n");
		set_tcon_cabc(1,0);
		alstate = 0;
		hdmi_tx_sysfs_enable_hpd(g_hdmi_ctrl, 1);
	} else {
		pr_err("[DISP] Enter Always on\n");
		set_tcon_cabc(1,1);
		alstate = 1;
		hdmi_tx_sysfs_enable_hpd(g_hdmi_ctrl, 0);
	}
	return 0;
}
...
static int mdss_dsi_unblank(struct mdss_panel_data *pdata)
{
	...
	if (ctrl_pdata->ndx == 0)
		enable_irq(gpio_to_irq(ctrl_pdata->lcd_err_gpio));
...
static int mdss_dsi_blank(struct mdss_panel_data *pdata, int power_state)
{
	...
	if (ctrl_pdata->ndx == 0)
		disable_irq(gpio_to_irq(ctrl_pdata->lcd_err_gpio));

	if (ctrl_pdata->ctrl_state & CTRL_STATE_PANEL_INIT) {
		if (!pdata->panel_info.dynamic_switch_pending) {
			if (ctrl_pdata->ndx == 0) {
				cancel_delayed_work_sync(&ctrl_pdata->lcd_err_work);
			}
...
static void issue_reset(void)
{
	...
	pinfo = &(mdss_dsi_res->ctrl_pdata[0]->panel_data.panel_info);
	for (i = 0; i < mdss_dsi_res->ctrl_pdata[0]->panel_data.panel_info.rst_seq_len; ++i) {
		gpio_direction_output((mdss_dsi_res->ctrl_pdata[0]->rst_gpio),
			mdss_dsi_res->ctrl_pdata[0]->panel_data.panel_info.rst_seq[i]);
		if (mdss_dsi_res->ctrl_pdata[0]->panel_data.panel_info.rst_seq[++i])
			usleep_range(pinfo->rst_seq[i] * 1000, pinfo->rst_seq[i] * 1000);
	}
}

static void mdss_reset_lcd(void)
{
	struct dsi_panel_cmds *on_cmds;
	//struct dcs_cmd_req cmdreq;
	//char led_null[1] = {0x00};	/* DTYPE_DCS_WRITE1 */
	//struct dsi_cmd_desc null_cmd = {
	//{DTYPE_DCS_WRITE, 1, 0, 0, 1, sizeof(led_null)},
	//led_null
	//};

	//mdss_dsi_panel_power_off(&(mdss_dsi_res->ctrl_pdata[0]->panel_data));
	//mdss_dsi_panel_power_off(&(mdss_dsi_res->ctrl_pdata[1]->panel_data));
	//msleep(10);
	//mdss_dsi_panel_power_on(&(mdss_dsi_res->ctrl_pdata[0]->panel_data));
	//mdss_dsi_panel_power_on(&(mdss_dsi_res->ctrl_pdata[1]->panel_data));
	//mdss_dsi_panel_reset(&(mdss_dsi_res->ctrl_pdata[0]->panel_data), 1);

	issue_reset();
	mdss_dsi_set_tx_power_mode(0, &(mdss_dsi_res->ctrl_pdata[0]->panel_data)); //hs
	mdss_dsi_set_tx_power_mode(0, &(mdss_dsi_res->ctrl_pdata[1]->panel_data)); //hs

	//mdss_dsi_set_tx_power_mode(1, &(mdss_dsi_res->ctrl_pdata[0]->panel_data)); //LP
	//mdss_dsi_set_tx_power_mode(1, &(mdss_dsi_res->ctrl_pdata[1]->panel_data)); //LP

	//memset(&cmdreq, 0, sizeof(cmdreq));
	//cmdreq.cmds = &null_cmd;
	//cmdreq.cmds_cnt = 1;
	//cmdreq.flags = CMD_REQ_COMMIT | CMD_CLK_CTRL;
	//cmdreq.rlen = 0;
	//cmdreq.cb = NULL;

	//mdss_dsi_cmdlist_put(mdss_dsi_res->ctrl_pdata[0], &cmdreq);
	on_cmds = &(mdss_dsi_res->ctrl_pdata[0]->on_cmds);

	pr_err("%s: ndx=%d cmd_cnt=%d\n", __func__,
				mdss_dsi_res->ctrl_pdata[0]->ndx, on_cmds->cmd_cnt);
	if (on_cmds->cmd_cnt)
		mdss_dsi_panel_cmds_send(mdss_dsi_res->ctrl_pdata[0], on_cmds, CMD_REQ_COMMIT | CMD_REQ_HS_MODE);

	mdss_dsi_res->ctrl_pdata[0]->ctrl_state |= CTRL_STATE_PANEL_INIT;
	mdss_dsi_res->ctrl_pdata[1]->ctrl_state |= CTRL_STATE_PANEL_INIT;
...
static char check_register_0x0E(void)
{
	struct dcs_cmd_req cmdreq;
	char rbuf[2];
	char read_status_mode[1] = {0x0E};
	struct dsi_cmd_desc read_status_cmd[] = {
		{{DTYPE_GEN_READ1,1,0,0,0,1},read_status_mode},
	};
	memset(&cmdreq, 0, sizeof(cmdreq));
...
static void mdss_dsi_lcd_err_work(struct work_struct *work)
{
	char rbuf_0A, rbuf_0E;
	int count = 0;

	printk("%s start\n", __func__);
	rbuf_0A = check_register_0x0A();
	pr_err("%s:0x0A status before:%02X\n", __func__, rbuf_0A);
	rbuf_0E = check_register_0x0E();
	pr_err("%s:0x0E status before:%02X\n", __func__, rbuf_0E);

	mdss_reset_lcd();
	mdss_dsi_panel_bklt_dcs(mdss_dsi_res->ctrl_pdata[0], last_bk_level);
	msleep(1500);
	while (check_register_0x0A() != 0x9C || check_register_0x0E() != 0x80) {
		mdss_reset_lcd();
		mdss_dsi_panel_bklt_dcs(mdss_dsi_res->ctrl_pdata[0], last_bk_level);
		msleep(1500);
		count++;
		pr_err("%s: reset lcd try %d\n", __func__, count);
		if (count==3)
			break;
	}
		pr_err("%s:0x0A status after:%02X\n", __func__, check_register_0x0A());
		pr_err("%s:0x0E status after:%02X\n", __func__, check_register_0x0E());
	printk("%s end\n", __func__);
}
...
static int mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	if (ctrl_pdata->ndx == 0) {
		gpio_request(ctrl_pdata->lcd_err_gpio, "lcd_error");
		if (rc) {
			pr_err("request lcd error gpio failed, rc=%d\n",
				rc);
			goto error_shadow_clk_deinit;
		}
		gpio_direction_input(ctrl_pdata->lcd_err_gpio);
		rc = devm_request_irq(&pdev->dev,
			gpio_to_irq(ctrl_pdata->lcd_err_gpio),
			lcd_err_handler, IRQF_TRIGGER_RISING | IRQF_ONESHOT,
			"LCD_ERR_GPIO", ctrl_pdata);
		if (rc) {
			pr_err("LCD error request_irq failed.\n");
			goto error_shadow_clk_deinit;
		}
		disable_irq(gpio_to_irq(ctrl_pdata->lcd_err_gpio));
	}
	...
	if (ctrl_pdata->ndx == 0) {
	ctrl_pdata->lcd_err_workq = create_singlethread_workqueue("mdss_lcd_err");
		if (!ctrl_pdata->lcd_err_workq) {
			pr_err("%s: Error creating single workqueue\n", __func__);
			rc = -EPERM;
			goto error_pan_node;
		}
	}
	...
	if (ctrl_pdata->ndx == 0)
		INIT_DELAYED_WORK(&ctrl_pdata->lcd_err_work, mdss_dsi_lcd_err_work);
	pr_err("%s: Dsi Ctrl->%d initialized, DSI rev:0x%x, PHY rev:0x%x, index:%d\n",
		__func__, index, ctrl_pdata->shared_data->hw_rev,
		ctrl_pdata->shared_data->phy_rev, index);
...
static int mdss_dsi_ctrl_remove(struct platform_device *pdev)
{
	...
	if (ctrl_pdata->ndx == 0) {
		if (ctrl_pdata->lcd_err_workq)
			destroy_workqueue(ctrl_pdata->lcd_err_workq);
	}
	...
	struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	ctrl_pdata->lcd_err_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			 "sdc,platform-lcd-err-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->lcd_err_gpio))
		pr_err("%s:%d, lcd_error gpio not specified\n",
						__func__, __LINE__);
