			...
			struct mdss_ad_init_cfg *init_cfg)
{
	...
	printk ( KERN_DEBUG "[DISP]  True2life Setting\n");
	printk ( KERN_DEBUG "[DISP]  i_control0 : %d\n",ad->init.i_control[0]);
	printk ( KERN_DEBUG "[DISP]  i_control1 : %d\n",ad->init.i_control[1]);
	printk ( KERN_DEBUG "[DISP]  black level : %d\n",ad->init.black_lvl);
	printk ( KERN_DEBUG "[DISP]  white level : %d\n",ad->init.white_lvl);
	printk ( KERN_DEBUG "[DISP]  varience : %d\n",ad->init.var);
	printk ( KERN_DEBUG "[DISP]  limit ampl : %d\n",ad->init.limit_ampl);
	printk ( KERN_DEBUG "[DISP]  i_dither : %d\n",ad->init.i_dither);
	printk ( KERN_DEBUG "[DISP]  slope max : %d\n",ad->init.slope_max);
	printk ( KERN_DEBUG "[DISP]  slope min : %d\n",ad->init.slope_min);
	printk ( KERN_DEBUG "[DISP]  dither_ctl : %d\n",ad->init.dither_ctl);
	printk ( KERN_DEBUG "[DISP]  format : %d\n",ad->init.format);
	printk ( KERN_DEBUG "[DISP]  auto size: %d\n",ad->init.auto_size);
	printk ( KERN_DEBUG "[DISP]  frame width : %d\n",ad->init.frame_w);
	printk ( KERN_DEBUG "[DISP]  frame height: %d\n",ad->init.frame_h);
...
int mdss_mdp_ad_input(struct msm_fb_data_type *mfd,
	...
	case MDSS_AD_MODE_MAN_STR:
		...
		if (input->in.strength > MDSS_MDP_MAX_AD_STR) {
			...
			if (input->in.strength == 257)
				printk(KERN_DEBUG"[DISP]Read current strength %d\n",ad->ad_data);
			else
				printk(KERN_DEBUG"[DISP]invalid input strength %d\n",input->in.strength);
			input->in.strength = ad->ad_data;
			ret = 1;
			goto error;
		}
		ad->ad_data_mode = MDSS_AD_INPUT_STRENGTH;
		printk(KERN_DEBUG"[DISP]strength = %d\n", input->in.strength);
