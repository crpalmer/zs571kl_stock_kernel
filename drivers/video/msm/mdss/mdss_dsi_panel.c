...
extern struct mdss_dsi_ctrl_pdata *g_ctrl_pdata_0;
...
extern enum DEVICE_EVBCFGID g_ASUS_evbcfgID;
...
static char vr_mode = 0;
static unsigned int max_bkl = 0;
#if 0
static char level1_unlock[3] = {0xF0, 0x5A, 0x5A};
static char level1_lock[3] = {0xF0, 0xA5, 0xA5};
#endif
...
static struct proc_dir_entry *vr_mode_switch;
static struct proc_dir_entry *lcd_uniqueID_proc_file;
static struct proc_dir_entry *max_bkl_proc_file;
static int bkl = 4;
extern enum DEVICE_DISPLAYID g_ASUS_displayID;
...
#if 0
static struct dsi_cmd_desc level1_unlock_cmd[] = {
    { {DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(level1_unlock)}, level1_unlock},
};
static struct dsi_cmd_desc level1_lock_cmd[] = {
    { {DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(level1_lock)}, level1_lock},
};
#endif
int last_bk_level;
			...
			struct dsi_panel_cmds *pcmds)
{
	...
	cmdreq.cmds = pcmds->cmds;
	cmdreq.cmds_cnt = pcmds->cmd_cnt;
	cmdreq.flags = CMD_REQ_COMMIT | CMD_REQ_HS_MODE; // | CMD_CLK_CTRL | CMD_REQ_HS_MODE | CMD_REQ_DMA_TPG;
	cmdreq.rlen = 0;
	cmdreq.cb = NULL;
...
#if 0
static char check_register_0xCB(void)
{
	struct dcs_cmd_req cmdreq;
	int i;
	char rbuf[66];
	char read_status_mode[1] = {0xCB};
	struct dsi_cmd_desc read_status_cmd[] = {
		{{DTYPE_GEN_READ1,1,0,0,0,1},read_status_mode},
	};
	memset(&cmdreq, 0, sizeof(cmdreq));
	cmdreq.cmds = read_status_cmd;
	cmdreq.cmds_cnt = 1;
	cmdreq.flags = CMD_REQ_RX | CMD_REQ_COMMIT;
	cmdreq.rlen = 66;
	cmdreq.rbuf = rbuf;
	cmdreq.cb = NULL;
	mdss_dsi_cmdlist_put(g_ctrl_pdata_0, &cmdreq);
	printk("%s ", __func__);
	for (i=0; i < 65; i++)
		printk("rbuf%d:%02X ", i, rbuf[i]);

	printk("\n");
	return rbuf[0];
}

static char check_register_0xB1(void)
{
	struct dcs_cmd_req cmdreq;
	char rbuf[16];
	char read_status_mode[1] = {0xB1};
	struct dsi_cmd_desc read_status_cmd[] = {
		{{DTYPE_GEN_READ1,1,0,0,0,1},read_status_mode},
	};
	memset(&cmdreq, 0, sizeof(cmdreq));
	...
	mdss_dsi_cmdlist_put(g_ctrl_pdata_0, &cmdreq);
		...
		printk("%s: rbuf0:%02X rbuf1:%02X rbuf2:%02X rbuf3:%02X rbuf4:%02X rbuf5:%02X rbuf6:%02X rbuf7:%02X rbuf8:%02X rbuf9:%02X rbuf10:%02X rbuf11:%02X rbuf12:%02X rbuf13:%02X rbuf14:%02X rbuf15:%02X\n"
		, __func__, rbuf[0], rbuf[1], rbuf[2], rbuf[3], rbuf[4], rbuf[5], rbuf[6], rbuf[7], rbuf[8], rbuf[9], rbuf[10], rbuf[11], rbuf[12], rbuf[13], rbuf[14], rbuf[15]);

	return rbuf[0];
}

static char check_register_0x0A(void)
{
	struct dcs_cmd_req cmdreq;
	char rbuf[2];
	char read_status_mode[1] = {0x0A};
	struct dsi_cmd_desc read_status_cmd[] = {
		{{DTYPE_GEN_READ1,1,0,0,0,1},read_status_mode},
	};
	memset(&cmdreq, 0, sizeof(cmdreq));
...
static void level1_key_unlock(void)
{
	struct dcs_cmd_req cmdreq;
	//char rbuf[2];
	//char read_status_mode[1] = {0xB1};
...
static void level1_key_lock(void)
{
	struct dcs_cmd_req cmdreq;
	//char rbuf[2];
	//char read_status_mode[1] = {0xB1};
...
static void check_lcd_uniqueid(void)
{
	struct dcs_cmd_req cmdreq;
	char rbuf[12];
	//char read_5byte_commands[1] = {0x05};
	char read_status_mode[2] = {0xD6, 0x05};
	//struct dsi_cmd_desc read_5byte_cmd[] = {
		//{{DTYPE_MAX_PKTSIZE,1,0,0,0,1},read_5byte_commands},
	//};
	struct dsi_cmd_desc read_status_cmd[] = {
		{{DTYPE_GEN_READ1,1,0,0,0,2},read_status_mode},
	};

	//memset(&cmdreq, 0, sizeof(cmdreq));
	//cmdreq.cmds = read_5byte_cmd;
	//cmdreq.cmds_cnt = 1;
	//cmdreq.flags = CMD_REQ_COMMIT | CMD_CLK_CTRL;
	//cmdreq.rlen = 0;
	//cmdreq.cb = NULL;
	...
	printk("%s: rbuf0:%02X rbuf1:%02X rbuf2:%02X rbuf3:%02X rbuf4:%02X rbuf5:%02X rbuf6:%02X rbuf7:%02X rbuf8:%02X rbuf9:%02X rbuf10:%02X rbuf11:%02X\n"
		, __func__, rbuf[0], rbuf[1], rbuf[2], rbuf[3], rbuf[4], rbuf[5], rbuf[6], rbuf[7], rbuf[8], rbuf[9], rbuf[10], rbuf[11]);
...
int set_tcon_cabc(int panel_reg, char mode)
{
	...
	switch (panel_reg){
		case CABC:
		    ...
		    if (g_ctrl_pdata_0->panel_data.panel_info.panel_power_state == MDSS_PANEL_POWER_ON) {
		...
		case ALPM:
			if (mode == 4)
				mdss_dsi_panel_cmds_send(g_ctrl_pdata_0, &alpm_change5_cmd, CMD_REQ_COMMIT);
			if (mode == 3)
				mdss_dsi_panel_cmds_send(g_ctrl_pdata_0, &alpm_change40_cmd, CMD_REQ_COMMIT);
			if (mode == 2)
				mdss_dsi_panel_cmds_send(g_ctrl_pdata_0, &alpm_on5_cmd, CMD_REQ_COMMIT);
			if (mode == 1)
			    mdss_dsi_panel_cmds_send(g_ctrl_pdata_0, &alpm_on_cmd, CMD_REQ_COMMIT);
			if (mode == 0)
			    mdss_dsi_panel_cmds_send(g_ctrl_pdata_0, &alpm_off_cmd, CMD_REQ_COMMIT);
			break;
#endif
		case HBM:
			if (dsi_power_state == 0) {
		...
		case VR:
			//if (mode == 3)
			    //mdss_dsi_panel_cmds_send(g_ctrl_pdata_0, &vr_on3_cmd, CMD_REQ_COMMIT);
			//if (mode == 2)
			    //mdss_dsi_panel_cmds_send(g_ctrl_pdata_0, &vr_on2_cmd, CMD_REQ_COMMIT);
			if (mode == 1) {
				//level1_key_unlock();
				//check_lcd_uniqueid();
				//rbuf_CB = check_register_0xCB();
				//rbuf_B1 = check_register_0xB1();
				//level1_key_lock();
				//check_register_0x0A();
				//printk("%s: register before AID: 0xCB:%02x, 0xB1:%02x\n", __func__, rbuf_CB, rbuf_B1);
			    mdss_dsi_panel_cmds_send(g_ctrl_pdata_0, &g_ctrl_pdata_0->lp_on_cmds, CMD_REQ_COMMIT);
				//level1_key_unlock();
				//rbuf_CB = check_register_0xCB();
				//rbuf_B1 = check_register_0xB1();
				//level1_key_lock();
				//check_register_0x0A();
				//printk("%s: register in AID: 0xCB:%02x, 0xB1:%02x\n", __func__, rbuf_CB, rbuf_B1);
			}
			else if (mode == 0) {
			    mdss_dsi_panel_cmds_send(g_ctrl_pdata_0, &g_ctrl_pdata_0->lp_off_cmds, CMD_REQ_COMMIT);
				//level1_key_unlock();
				//rbuf_CB = check_register_0xCB();
				//rbuf_B1 = check_register_0xB1();
				//level1_key_lock();
				//check_register_0x0A();
				//printk("%s: register after exiting AID: 0xCB:%02x, 0xB1:%02x\n", __func__, rbuf_CB, rbuf_B1);
			}
			break;
		default:;
		}
...
#if 0
static char nop[1] = {0x00};	/* DTYPE_DCS_WRITE */
static struct dsi_cmd_desc nop_cmd = {
	{DTYPE_DCS_WRITE, 1, 0, 0, 1, sizeof(nop)},
	nop
};
#endif
...
static void mdss_dsi_panel_under_max_bklt(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	struct dcs_cmd_req cmdreq;
							...
							u32 mode)
{
	...
	if ((mode & DISPLAY_LOW_PERSISTENCE_MASK) && (lp_on_cmds->cmd_cnt)) {
		...
		mdss_dsi_panel_bklt_dcs(ctrl, last_bk_level);
	}
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	printk("%s: dispaly-on commands done\n", __func__);
...
static int mdss_dsi_panel_off(struct mdss_panel_data *pdata)
{
	...
	printk("%s: dispaly-off commands done\n", __func__);
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	mdss_dsi_parse_dcs_cmds(np, &alpm_on_cmd,
		"qcom,mdss-dsi-alpm-on-command", "qcom,mdss-dsi-off-command-state");
	mdss_dsi_parse_dcs_cmds(np, &alpm_off_cmd,
		"qcom,mdss-dsi-alpm-off-command", "qcom,mdss-dsi-off-command-state");
	mdss_dsi_parse_dcs_cmds(np, &alpm_on5_cmd,
		"qcom,mdss-dsi-alpm-on5-command", "qcom,mdss-dsi-off-command-state");
	mdss_dsi_parse_dcs_cmds(np, &alpm_change40_cmd,
	"qcom,mdss-dsi-alpm-change40-command", "qcom,mdss-dsi-off-command-state");
	mdss_dsi_parse_dcs_cmds(np, &alpm_change5_cmd,
		"qcom,mdss-dsi-alpm-change5-command", "qcom,mdss-dsi-off-command-state");

	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->lp_on_cmds,
		"qcom,mdss-dsi-lp-mode-on", "qcom,mdss-dsi-off-command-state");

	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->lp_off_cmds,
		"qcom,mdss-dsi-lp-mode-off", "qcom,mdss-dsi-off-command-state");
...
static ssize_t vr_mode_proc_write(struct file *file, const const char __user *buff, size_t count,loff_t *ops)
{
	char temp;
	if(count > 0)
	{
	...
	printk("[DISPLAY] : The Current VR Mode is %d\n",vr_mode);
	set_tcon_cabc(VR, vr_mode);

	return count;
}

static ssize_t max_bkl_proc_write(struct file *file, const const char __user *buff, size_t count,loff_t *ops)
{
	char temp[4];
	int res;
	if (g_mfd->panel_power_state == MDSS_PANEL_POWER_OFF)
		return count;
	if(count > 0)
	{
		if(copy_from_user(temp,buff, count))
			return -EFAULT;
		temp[count] = '\0';
		res = kstrtou32(temp, 0, &max_bkl);
		if(max_bkl > 255 || max_bkl < 0){
			printk("[DISPLAY] : The error number\n");
			return -1;
		} else {
			//vr_mode = temp - 0x30;
			if (max_bkl == 0) {
				mdss_dsi_panel_under_max_bklt(g_ctrl_pdata_0, last_bk_level);
			}
			else if (last_bk_level > max_bkl) {
				mdss_dsi_panel_under_max_bklt(g_ctrl_pdata_0, max_bkl);
			}
		}
	}
	printk("[DISPLAY] : The Current max bkl is %d\n",max_bkl);
	//set_tcon_cabc(VR, vr_mode);
...
static int vr_mode_proc_read(struct seq_file *buf, void *v)
{
	int temp = 0;
	if(vr_mode)
		//temp = 75 + 5*vr_mode;
		temp = 90;
	seq_printf(buf, "vr_mode :%d, AOR: %d\n", vr_mode,temp);
	return 0;
}

static int max_bkl_proc_read(struct seq_file *buf, void *v)
{
	//int temp = 0;
	if(max_bkl)
		//temp = 75 + 5*vr_mode;
		//temp = 90;
		seq_printf(buf, "max_bkl :%d\n", max_bkl);
	return 0;
}
...
static int vr_mode_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file,vr_mode_proc_read,NULL);
}

static int max_bkl_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file,max_bkl_proc_read,NULL);
}
...
static struct file_operations vr_mode_proc_ops = {
	.open = vr_mode_proc_open,
	.read = seq_read,
	.write = vr_mode_proc_write,
	.release = single_release,
};

static struct file_operations max_bkl_proc_ops = {
	.open = max_bkl_proc_open,
	.read = seq_read,
	.write = max_bkl_proc_write,
	.release = single_release,
};
...
static void create_vr_mode_file(void)
{	char name[] = "vr_mode";
	printk("[DISPLAY] : create vr_mode file\n");
	vr_mode_switch = proc_create(name,0666,NULL,&vr_mode_proc_ops);
	if(vr_mode_switch){
        printk("[DISPLAY] : create_vr_mode_file sucessed!\n");
    }else{
		printk("[DISPLAY] : create create_vr_mode_file failed!\n");
    }
}

static void create_max_bkl_file(void)
{
	char name[] = "max_bkl";
	printk("[DISPLAY] : create max_bkl file\n");
	max_bkl_proc_file = proc_create(name,0666,NULL,&max_bkl_proc_ops);
	if(vr_mode_switch){
		printk("[DISPLAY] : create_max_bkl_file sucessed!\n");
	} else {
		printk("[DISPLAY] : create max_bkl_file failed!\n");
	}
}
