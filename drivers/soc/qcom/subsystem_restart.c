...
#define SUBSYS_NAME_TAG "SUBSYS_NAME"/*ASUS-BBSP Send SubSys UEvent+*/
#define SUBSYS_REASON_TAG "SUBSYS_REASON"/*ASUS-BBSP Send SubSys UEvent+*/
...
static void subsystem_restart_wq_func(struct work_struct *work)
{
	...
	if (get_ssr_enable_ramdumps()) {/*ASUS-BBSP Skip ramdump or panic in a specific reason+*/
		/*ASUS-BBSP Send SubSys UEvent - Subsys Offline+++*/
		...
		kobject_uevent_env(&(dev->desc->dev->kobj), KOBJ_OFFLINE, envp);
		/*ASUS-BBSP Send SubSys UEvent - Subsys Offline---*/
	}
	...
	/*ASUS-BBSP Send SubSys UEvent - Subsys Crash Reason+++*/
	snprintf(mName_Buf, sizeof(mName_Buf), "%s=%s", SUBSYS_NAME_TAG, desc->name);
	...
	if (get_ssr_enable_ramdumps()) {/*ASUS-BBSP Skip ramdump or panic in a specific reason+*/
		/*ASUS-BBSP Send SubSys UEvent - Subsys Online+++*/
		snprintf(mName_Buf, sizeof(mName_Buf), "%s=%s", SUBSYS_NAME_TAG, desc->name);
		snprintf(mReason_Buf, sizeof(mReason_Buf), "%s=[SSR]:%s %s", SUBSYS_REASON_TAG, desc->name, ssr_reason);
		kobject_uevent_env(&(dev->desc->dev->kobj), KOBJ_ONLINE, envp);
		/*ASUS-BBSP Send SubSys UEvent - Subsys Online---*/
	}
