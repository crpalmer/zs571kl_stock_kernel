				...
				size_t count, loff_t *ppos)
{
	...
	if (!strcmp(access_str, "slimslave_peek")) {
		...
		snprintf(lbuf, sizeof(lbuf), "%s\n",
				gpio_get_value(pdata->uart_control)? "uart": "hp");
		ret_cnt = simple_read_from_buffer(ubuf, count, ppos, lbuf,
	...
	const char __user *ubuf, size_t cnt, loff_t *ppos)
{
	...
	if (!strcmp(access_str, "slimslave_poke")) {
		...
		param[0] = (long int)(lbuf[0] - '0');
		pdata = debugCodec->slim->dev.platform_data;
		pr_info("param %ld\n", param[0]);
		/* if fastboot uart was off, never switch to uart */
		gpio_set_value(pdata->uart_control, (uart_info && (param[0] != 0))? 1: 0);
	...
	const char *name, u32 *val)
{
	...
	if (ret)
		dev_warn(dev, "Looking up %s property in node %s, no entry",
...
static struct wcd9xxx_pdata *wcd9xxx_populate_dt_pdata(struct device *dev)
{
	...
	ret = gpio_request_one(pdata->uart_control,
			uart_info ? GPIOF_OUT_INIT_HIGH: GPIOF_OUT_INIT_LOW,
			"uart-sw-control");
