...
static int asustek_fingerprintkey_pinctrl_configure(struct pinctrl *key_pinctrl, bool active)
{
	...
	if (active) {
		set_state = pinctrl_lookup_state(key_pinctrl,"asustek_fingerprintkey_active");

		if (IS_ERR(set_state)) {
			pr_err("Can't get asustek_fingerprintkey pinctrl active state\n");
			return PTR_ERR(set_state);
		}
	} else {
		set_state = pinctrl_lookup_state(key_pinctrl, "asustek_fingerprintkey_suspend");

		if (IS_ERR(set_state)) {
			pr_err("Can't get asustek_fingerprintkey pinctrl sleep state\n");
			return PTR_ERR(set_state);
		}
	}
	...
	if (retval) {
		pr_err("Can't set asustek_fingerprintkey pinctrl state\n");
		return retval;
	}
	return 0;
}
...
static int __init fingerprintkey_driver_probe(struct platform_device *pdev)
{
	...
	if (ddata->key_pinctrl) {
		...
		ret = asustek_fingerprintkey_pinctrl_configure(ddata->key_pinctrl, true);
		if (ret) {
