...
unsigned int save_fw_id;
unsigned char save_config_id[32] = {0};

static int synaptics_dsx_regulator_enable(struct synaptics_rmi4_data
			*rmi4_data, bool on);
/*static int synaptics_dsx_regulator_vdd_enable(struct synaptics_rmi4_data
			*rmi4_data, bool on);
static int synaptics_dsx_regulator_avdd_enable(struct synaptics_rmi4_data
			*rmi4_data, bool on);*/
...
static ssize_t synaptics_rmi4_regulator_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
static ssize_t synaptics_rmi4_proxi_notify_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_rmi4_proxi_notify_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(regulator, (S_IWUSR | S_IWGRP),
			synaptics_rmi4_show_error,
			synaptics_rmi4_regulator_store),
	__ATTR(suspend, 0660,
			synaptics_rmi4_show_error,
			synaptics_rmi4_suspend_store),
	__ATTR(proxi_notify, S_IRUGO | S_IWUSR,
                        synaptics_rmi4_proxi_notify_show,
                        synaptics_rmi4_proxi_notify_store),
		...
		struct device_attribute *attr, char *buf)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			...
			&ctrl23_control,
			sizeof(ctrl23_control));
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to read f12 ctrl23\n",
				__func__);
	}

	pr_info("[Synaptics] glove enabled = %d, ctrl23_gloved = 0x%x\n",
			rmi4_data->enable_gloved, (ctrl23_control & 0x20));

	return scnprintf(buf, PAGE_SIZE,
		"[Synaptics] glove enabled = %d, ctrl23_gloved = 0x%x\n",
			rmi4_data->enable_gloved, (ctrl23_control & 0x20));
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "[Synaptics] cover mode = %d\n",
					rmi4_data->enable_cover_mode);
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	rmi4_data->enable_cover_mode = input;
	...
	if (!rmi4_data->auto_update_finish) {
		...
		else if (rmi4_data->enable_cover_mode == 0)
		{
			if (rmi4_data->enable_gloved == 1) {
				synaptics_rmi4_f12_gloved(rmi4_data, 1);
			} else if (rmi4_data->enable_gloved == 0) {
				synaptics_rmi4_f12_gloved(rmi4_data, 0);
			}
		}
	}
	...
	pr_info("[Synaptics] touch cover_mode = %d\n", input);
	return count;
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (buf[0] == '1')
		zenMotion_dclick = true;
	else	/*disabled if input unknown*/
		zenMotion_dclick = false;

	if (zenMotion_dclick || zenMotion_swipeup ||
		((zenMotion_gesture & 0x40) && (zenMotion_gesture & 0xbf)))
	{
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (buf[0] == '1')
		zenMotion_swipeup = true;
	else	/*disabled if input unknown*/
		zenMotion_swipeup = false;
		...
		struct device_attribute *attr, char *buf)
{
	//struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	return scnprintf(buf, PAGE_SIZE,
		"[Synaptics] Gesture m-wseczv = %d-%d %d %d %d %d %d\n",
				(zenMotion_gesture & 0x40) >> 6,
				(zenMotion_gesture & 0x20) >> 5,
				(zenMotion_gesture & 0x10) >> 4,
				(zenMotion_gesture & 0x08) >> 3,
				(zenMotion_gesture & 0x04) >> 2,
				(zenMotion_gesture & 0x02) >> 1,
				(zenMotion_gesture & 0x01));
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_info("[Synaptics] %s: set gesture m-wseczv = %d-%d %d %d %d %d %d\n",
				__func__,
				(zenMotion_gesture & 0x40) >> 6,
				(zenMotion_gesture & 0x20) >> 5,
				(zenMotion_gesture & 0x10) >> 4,
						...
						unsigned char swipe_x_dist)
{
	if (gesture_type == SYNA_DCLICK_TYPE) {
		...
		pr_info("[Synaptics] gesture_type = %x\n", gesture_type);
		switch (gesture_id) {
		case SYNA_GESTURE_W:
			if (zenMotion_gesture & 0x20) {
...
// add by leo for proximity sensor notify ++
static ssize_t synaptics_rmi4_proxi_notify_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
        struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
                ...
                struct device_attribute *attr, const char *buf, size_t count)
{
        ...
        wake_lock_timeout(&rmi4_data->synaptics_wakelock, 3 * HZ);
        //cancel_delayed_work_sync(&rmi4_data->irq_work);

        rmi4_data->proxi_notify = input > 0 ? true : false;
        pr_info("[Synaptics] %s: rmi4_data->proxi_notify = %s\n", __func__, rmi4_data->proxi_notify?"ture":"false");
        queue_delayed_work(rmi4_data->proxi_notify_workqueue, &rmi4_data->proxi_notify_work, 0 * HZ);
...
static irqreturn_t synaptics_rmi4_irq(int irq, void *data)
{
	...
	//pr_info("[Synaptics] synaptics_rmi4_irq!!!!!\n");
	wake_lock_timeout(&rmi4_data->synaptics_wakelock, 1 * HZ);
...
flash_prog_mode:
	...
	dev_info(rmi4_data->pdev->dev.parent, "FW build ID = %u\n",
						rmi4_data->firmware_id);
	save_fw_id = rmi4_data->firmware_id;
	...
	if (fwu) {
		...
		dev_info(rmi4_data->pdev->dev.parent, "FW build CONFIG = %02x%02x%02x%02x\n",
						config_id[0], config_id[1],
						config_id[2], config_id[3]);
	}

	for(i=0; i<32; i++) {
		save_config_id[i] = config_id[i];
	}
...
static int synaptics_rmi4_set_gpio(struct synaptics_rmi4_data *rmi4_data)
{
	...
	pr_info("[Synaptics] synaptics_rmi4_set_gpio begin\n");
	//pr_info("[Synaptics] irq = %d !!!\n", gpio_get_value(bdata->irq_gpio));
	...
	if (bdata->reset_gpio >= 0) {
		...
		pr_info("[Synaptics] reset init high\n");
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
	...
	/*pr_info("[Synaptics] reset = %d !!!\n", gpio_get_value(bdata->reset_gpio));
	pr_info("[Synaptics] irq = %d !!!\n", gpio_get_value(bdata->irq_gpio));
	if (bdata->reset_gpio >= 0) {
		gpio_set_value(bdata->reset_gpio, bdata->reset_on_state);
		pr_info("[Synaptics] set reset_gpio low !!!\n");
		pr_info("[Synaptics] reset = %d !!!\n", gpio_get_value(bdata->reset_gpio));
		msleep(bdata->reset_active_ms);
		pr_info("[Synaptics] irq = %d !!!\n", gpio_get_value(bdata->irq_gpio));
		gpio_set_value(bdata->reset_gpio, !bdata->reset_on_state);
		pr_info("[Synaptics] set reset_gpio high !!!\n");
		pr_info("[Synaptics] reset = %d !!!\n", gpio_get_value(bdata->reset_gpio));
...
// add by leo for proximity sensor notify ++
static void synaptics_rmi4_proxi_notify_work(struct work_struct *work)
{
	struct delayed_work *delayed_work =
			container_of(work, struct delayed_work, work);
	struct synaptics_rmi4_data *rmi4_data =
        ...
        if (rmi4_data->proxi_notify) {
                if (!rmi4_data->irq_enabled) {
			if (rmi4_data->suspend) {
				pr_info("[Synaptics] %s: wait for system resume\n", __func__);
			}
			else if (!rmi4_data->suspend) {
				synaptics_rmi4_irq_enable(rmi4_data, true, false);
				rmi4_data->irq_enabled = true;
				pr_info("[Synaptics] %s: Enable touch irq\n", __func__);
			}
                } else
                        pr_info("[Synaptics] %s: Touch irq already enabled\n", __func__);
        } else {
                if (rmi4_data->irq_enabled) {
			if (!rmi4_data->suspend) {
				synaptics_rmi4_irq_enable(rmi4_data, false, false);
				rmi4_data->irq_enabled = false;
				pr_info("[Synaptics] %s: Disable touch irq\n", __func__);
			}
			else
				pr_info("[Synaptics] %s: Touch in suspend\n", __func__);
                } else
                        pr_info("[Synaptics] %s: Touch irq already disabled\n", __func__);
        }
...
static ssize_t synaptics_touch_switch_name(struct switch_dev *sdev, char *buf)
{
	...
	if (rmi4_data->suspend == true) {
		pr_info("[Synaptics] firmware ID = %u\n", save_fw_id);
		count += sprintf(buf + count, "%u", save_fw_id);
	}
	else {
		retval = synaptics_rmi4_reg_read(rmi4_data,
			rmi4_data->f01_query_base_addr + F01_BUID_ID_OFFSET,
	...
	if (rmi4_data->suspend == true) {
		pr_info("[Synaptics] config ID = %02x%02x%02x%02x\n",
								save_config_id[0], save_config_id[1],
								save_config_id[2], save_config_id[3]);
...
{
	int retval;
	//int num_vdd, num_avdd;

	pr_info("[Synaptics] %s: start\n", __func__);

	rmi4_data->regulator_vdd = regulator_get(rmi4_data->pdev->dev.parent, "vdd");
	...
	if (regulator_count_voltages(rmi4_data->regulator_vdd) > 0) {
		//num_vdd = (regulator_count_voltages(rmi4_data->regulator_vdd));
		//pr_info("[Synaptics] regulator_count_vdd=%d\n",  num_vdd);

		retval = regulator_set_voltage(rmi4_data->regulator_vdd, 1800000, 1800000);
		if (retval) {
			pr_err("set voltage level for vdd l14 fail\n");
			regulator_set_voltage(rmi4_data->regulator_vdd, 0, 1800000);
			return retval;
		}
	}
	...
	if (regulator_count_voltages(rmi4_data->regulator_avdd) > 0) {
		//num_avdd = (regulator_count_voltages(rmi4_data->regulator_avdd));
		//pr_info("[Synaptics] regulator_count_avdd=%d\n",  num_avdd);

		retval = regulator_set_voltage(rmi4_data->regulator_avdd , 3300000, 3300000);
		if (retval) {
			pr_err("set voltage level for vdd l22 fail\n");
			regulator_set_voltage(rmi4_data->regulator_vdd, 0, 3300000);
			return retval;
		}
	}
			...
			*rmi4_data, bool on)
{
	...
	pr_info("[Synaptics] %s: en = %s\n", __func__, (on)?"enable":"disable");
	if (on) {
		pr_info("[Synaptics] %s: regulator enable\n", __func__);
		...
		pr_info("[Synaptics] %s: regulator disable\n", __func__);
		retval = regulator_disable(rmi4_data->regulator_vdd);
		if (retval) {
			dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to disable regulator vdd\n",
				__func__);
			return retval;
		}
		retval = regulator_disable(rmi4_data->regulator_avdd);
		if (retval) {
			dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to disable regulator avdd\n",
				__func__);
			return retval;
		}
	}
                        ...
                        *rmi4_data, bool on)
{
	...
	pr_info("[Synaptics] %s: en = %s\n", __func__, (on)?"enable":"disable");
	if (on) {
		pr_info("[Synaptics] %s: vdd l14 regulator enable\n", __func__);
		retval = regulator_enable(rmi4_data->regulator_vdd);
		if (retval) {
			dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to enable regulator vdd\n",
		...
		pr_info("[Synaptics] %s: vdd l14 regulator disable\n", __func__);
		retval = regulator_disable(rmi4_data->regulator_vdd);
		if (retval) {
			dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to disable regulator vdd\n",
                        ...
                        *rmi4_data, bool on)
{
	...
	pr_info("[Synaptics] %s: en = %s\n", __func__, (on)?"enable":"disable");
	if (on) {
		pr_info("[Synaptics] %s: avdd l22 regulator enable\n", __func__);
		retval = regulator_enable(rmi4_data->regulator_avdd);
		if (retval) {
			dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to enable regulator avdd\n",
		...
		pr_info("[Synaptics] %s: avdd l22 regulator disable\n", __func__);
		retval = regulator_disable(rmi4_data->regulator_avdd);
		if (retval) {
			dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to disable regulator avdd\n",
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	pr_info("[Synaptics] probe function begin\n");

	if(g_charger_mode == 1) {
		pr_info("%s:[%d]: In COS, Skip Probe\n", __func__, __LINE__);
		return 0;
	}
	...
	rmi4_data->suspend = false;
	rmi4_data->always_on_panel = false;
	rmi4_data->irq_enabled = false;
	rmi4_data->fingers_on_2d = false;
	...
	rmi4_data->enable_cover_mode = false;
	...
	pr_info("[Synaptics] %s: regulator l14 l22 enable\n", __func__);
	...
	if (g_ASUS_hwID == 0) {
		...
		if (!retval && rmi4_data->ts_pinctrl) {
			pr_info("[Synaptics] pinctrl : irq pull high !!!\n");
...
#ifdef CONFIG_FB
	pr_info("[Synaptics] CONFIG_FB yes\n");
...
#ifdef USE_EARLYSUSPEND
	pr_info("[Synaptics] USE_EARLYSUSPEND yes\n");
	...
	pr_info("[Synaptics] enable irq ok\n");
	pr_info("[Synaptics] irq = %d !!!\n", gpio_get_value(bdata->irq_gpio));
	...
	pr_info("[Synaptics] create sysfs attributes ok\n");
	...
	pr_info("[Synaptics] create platform_dev sysfs attributes ok\n");
	...
	// add by leo for proximity sensor notify ++
	wake_lock_init(&rmi4_data->synaptics_wakelock, WAKE_LOCK_SUSPEND, "synaptics-touchscreen");
	rmi4_data->proxi_notify_workqueue =
			create_singlethread_workqueue("dsx_proxi_notify_workqueue");
	INIT_DELAYED_WORK(&rmi4_data->proxi_notify_work, synaptics_rmi4_proxi_notify_work);
	// add by leo for proximity sensor notify --
	...
	pr_info("[Synaptics] probe function end\n");
	return retval;
...
void synaptics_rmi4_shutdown(struct platform_device *pdev)
{
	...
	if(g_charger_mode == 1) {
		pr_info("%s:[%d]: In COS, Skip Shutdown\n", __func__, __LINE__);
	}
	else {
		...
		retval = synaptics_dsx_regulator_enable(rmi4_data, false);
		if (retval < 0)
			printk("[Synaptics] %s: set regulator disable failed\n", __func__);
		else
			printk("[Synaptics] %s: regulator disable\n", __func__);
	}
}
		...
		bool enable)
{
	...
	struct synaptics_rmi4_fn *fhandler;
	struct synaptics_rmi4_device_info *rmi;
	unsigned char ctrl10[CTRL10_NUM] = {0};

	printk("[Synaptics] synaptics_rmi4_f12_gloved\n");
	...
	extra_data = (struct synaptics_rmi4_f12_extra_data *)fhandler->extra;
	offset = extra_data->ctrl23_offset;
	offset2 = extra_data->ctrl10_offset;
	...
	/*pr_info("%s: ctrl23 offset = 0x%x, addr = 0x%x, value = 0x%x\n",
			__func__, offset,
			fhandler->full_addr.ctrl_base + offset,
			ctrl23_control);*/

	retval = synaptics_rmi4_reg_read(rmi4_data,
			fhandler->full_addr.ctrl_base + offset2,
			ctrl10, sizeof(ctrl10));
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to read crtl10_data\n",
				__func__);
		return;
	}
	/*pr_info("%s: ctrl10 noise floor: addr = 0x%x, value = 0x%x\n",
			__func__,
			fhandler->full_addr.ctrl_base + offset2,
			ctrl10[0]);
	pr_info("%s: ctrl10 min peak amplitude: addr = 0x%x, value = 0x%x\n",
			__func__,
			fhandler->full_addr.ctrl_base + offset2,
			ctrl10[1]);*/
	...
	pr_info("%s: ctrl23 set value = 0x%x\n",
			__func__, ctrl23_control);
	...
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to change enable/disable gloved mode\n",
				__func__);
		return;
	}

	pr_info("%s: ctrl10[0] set value = 0x%x, ctrl10[1] set value = 0x%x\n",
			__func__, ctrl10[0], ctrl10[1]);
	...
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to change ctrl10 gloved mode\n",
				__func__);
		return;
	}
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	if (!rmi4_data->suspend) {
		printk("[Synaptics] %s: enter sleep mode\n", __func__);
		synaptics_rmi4_irq_enable(rmi4_data, false, false);
		synaptics_rmi4_sleep_enable(rmi4_data, true);
		synaptics_rmi4_free_fingers(rmi4_data);

		if (!rmi4_data->always_on_panel) {
			printk("[Synaptics] power off");
			retval = synaptics_dsx_regulator_enable(rmi4_data, false);
			if (retval < 0)
				printk("[Synaptics] %s: set regulator disable failed\n", __func__);
			else
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	if (g_ASUS_displayID == 0) {
		if (!rmi4_data->auto_update_finish) {
			...
			retval = synaptics_dsx_regulator_enable(rmi4_data, true);
			if (retval < 0)
				printk("[Synaptics] %s: set regulator enable failed\n", __func__);
			else
				printk("[Synaptics] %s: regulator enable\n", __func__);
			return 0;
		}
	}
	...
	if (rmi4_data->enable_wakeup_gesture) {
		...
		if (!rmi4_data->always_on_panel) {
			printk("[Synaptics] power on");
			if (bdata->reset_gpio != 1) {
				pr_info("[Synaptics] reset = %d !!!\n", gpio_get_value(bdata->reset_gpio));
	...
	if ((rmi4_data->enable_gloved == 1) || (rmi4_data->enable_cover_mode == 1))
