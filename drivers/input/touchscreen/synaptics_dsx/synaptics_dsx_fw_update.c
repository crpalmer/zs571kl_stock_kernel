...
extern unsigned int save_fw_id;
extern unsigned char save_config_id[32];
...
static struct device_attribute attrs[] = {
	...
	__ATTR(blconfigblockcount, 0660,
			...
			synaptics_rmi4_store_error),
	__ATTR(guestcodeblockcount, 0660,
...
exit:
	...
	pr_info("[Synaptics] FW build ID = %u\n", rmi4_data->firmware_id);
	...
	if (fwu) {
		...
		pr_info("[Synaptics] config build ID = %02x%02x%02x%02x\n",
