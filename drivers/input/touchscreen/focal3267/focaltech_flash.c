...
#define FTXXXX_INI_FILEPATH_CONFIG "/data"

// add by leo ++
#define DEFAULT_CHIP_ID_INDEX 5
...
int fts_6336GU_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	/*********Step 7: reset the new FW***********************/
	FTS_DBG("FT3267 Step 7: reset the new FW\n");
...
int fts_ctpm_fw_upgrade_with_app_file(struct i2c_client *client, char *firmware_name)
{
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x33))
        {
                i_ret = fts_6336GU_ctpm_fw_upgrade(client, pbt_buf, fwsize);
        }
...
int fts_ctpm_get_i_file_ver(void)
{
	...
	if (ui_sz > 2)
	{
	    //if(fts_updateinfo_curr.CHIP_ID==0x36  || fts_updateinfo_curr.CHIP_ID==0x86  || fts_updateinfo_curr.CHIP_ID==0x64 ||fts_updateinfo_curr.CHIP_ID==0x33)
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x64)||(fts_updateinfo_curr.CHIP_ID==0x33))
	{
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x86))
	{
		...
		dev_err(&client->dev, "[FTS]No this chip ID !.\n");
		}
	return i_ret;
}
/************************************************************************
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	if (uc_tp_fm_ver == FTS_REG_FW_VER ||	uc_tp_fm_ver != uc_host_fm_ver )
	{
		...
		printk("[FOCAL] %s : no need to upgrade\n", __func__);
	}
	printk("[FOCAL] %s end\n", __func__);
	return 0;
}

int fts_sw_reset(struct i2c_client *client)
{
	u8 reg_val[2] = {0};
	u32 i = 0;
	u8 auc_i2c_write_buf[10];
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++)
	{
		...
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1
			&& reg_val[1] == fts_updateinfo_curr.upgrade_id_2)
		{
			FTS_DBG("[FTS] sw reset Step 1: GET CTPM ID OK,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
			break;
		}
		else
	...
	/*********Step 7: reset the new FW***********************/
	FTS_DBG("[FTS] sw reset Step 2: reset the FW\n");
