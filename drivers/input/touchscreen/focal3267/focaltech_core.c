...
#define KEY_APP_SWITCH_X 80
#define KEY_APP_SWITCH_Y 900

#define KEY_BACK_X 240
#define KEY_BACK_Y 900
...
int     vk_3v3;
void focal3267_reset_ic(void); // add by leo
...
static struct qpnp_pin_cfg mpps_test_en =
{
.mode = QPNP_PIN_MODE_DIG_OUT,
.cs_out = QPNP_PIN_CS_OUT_5MA,
.src_sel = 1, //for MPPs, this is the enable select
.master_en = QPNP_PIN_MASTER_ENABLE,
};

static struct qpnp_pin_cfg mpps_test_dis =
{
.mode = QPNP_PIN_MODE_DIG_OUT,
.cs_out = QPNP_PIN_CS_OUT_5MA,
.src_sel = 1, //for MPPs, this is the enable select
...
void led_configure(int enable){
	if(enable>0){
		qpnp_pin_config(qpnp_mpps_1, &mpps_test_en);
		ledstatus=enable;
		printk("[LED] led on !!!\n");
	}
	else{
		qpnp_pin_config(qpnp_mpps_1, &mpps_test_dis);
		ledstatus=0;
		printk("[LED] led off !!!\n");
	}
...
static void fts_report_value(struct fts_ts_data *data)
{
	...
	{
        //printk("[%s] event type:%d\n",__func__,event->au8_touch_event[i]);
        //printk("[%s] x:%d y:%d\n",__func__,event->au16_x[i],event->au16_y[i]);
		if ((event->au8_touch_event[i] == FTS_TOUCH_DOWN))
		{
			if( (event->au16_x[i] == KEY_APP_SWITCH_X) && (event->au16_y[i] == KEY_APP_SWITCH_Y) )
			{
				if (switch_mode == 0){
					//printk("[FOCAL] switch_mode=0\n");
					printk("[%s] APP_SWITCH DOWN\n", __func__);
					input_report_key(fts_input_dev, APP_SWITCH, 1);
					input_sync(fts_input_dev);
					return;
				}else if (switch_mode == 1){
					//printk("[FOCAL] switch_mode=1\n");
					printk("[%s] KEY_BACK DOWN\n", __func__);
					input_report_key(fts_input_dev, KEY_BACK, true);
					input_sync(fts_input_dev);
					return;
				}
			}

			if( (event->au16_x[i] == KEY_BACK_X) && (event->au16_y[i] == KEY_BACK_Y) )
			{
				if (switch_mode == 0){
					//printk("[FOCAL] switch_mode=0\n");
					printk("[%s] KEY_BACK DOWN\n", __func__);
		...
		if ((event->au8_touch_event[i] == FTS_TOUCH_UP))
		{
			if( (event->au16_x[i] == KEY_APP_SWITCH_X) && (event->au16_y[i] == KEY_APP_SWITCH_Y) )
			{
				if (switch_mode == 0){
					//printk("[FOCAL] switch_mode=0\n");
					printk("[%s] APP_SWITCH UP\n", __func__);
					input_report_key(fts_input_dev, APP_SWITCH, 0);
					input_sync(fts_input_dev);
					return;
				}else if (switch_mode == 1){
					//printk("[FOCAL] switch_mode=1\n");
					printk("[%s] KEY_BACK UP\n", __func__);
					input_report_key(fts_input_dev, KEY_BACK, false);
					input_sync(fts_input_dev);
					return;
				}
			}
...
static int fts_gpio_configure(struct fts_ts_data *data, bool on)
{
	...
	if (on) {
		...
		if (gpio_is_valid(data->pdata->reset_gpio)) {
			...
			{
				printk("///////////////////		[%s] fts_reset_gpio request successed. (reset_gpio = %d)", __func__, data->pdata->reset_gpio);
			}
...
static int fts_ts_start(struct device *dev)
{
	...
	msleep(20);
	if(err<0)
		printk("////////////////////		[%s] write a5 00 fail\n", __func__);
	else
		printk("////////////////////		[%s] write a5 00 success\n", __func__);
...
static int fts_ts_stop(struct device *dev)
{
	...
	if (gpio_is_valid(data->pdata->reset_gpio)) {
		...
		msleep(20);
		if(err<0)
			printk("////////////////////		[%s] write a5 03 fail\n", __func__);
		else
			printk("////////////////////		[%s] write a5 03 success\n", __func__);
	}

	data->suspended = true;
	printk("[FOCAL] suspend done\n");
	return 0;
}
...
int fts_ts_resume(struct device *dev)
{
	...
	if(gloved_mode == 1){
		printk("[Focal] gloved_mode = 1\n");
		txbuf[0] = 0xC0;
		txbuf[1] = 1;
		err = fts_i2c_write(data->client, txbuf, sizeof(txbuf));
		msleep(20);
		if(err < 0)
			printk("[Focal] %s : write C0 01 fail\n", __func__);
		else
			printk("[Focal] %s : write C0 01 success\n", __func__);
	}

	printk("[FOCAL] resume done\n");
	return 0;
}
				 ...
				 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK &&
		...
		if (*blank == FB_BLANK_POWERDOWN) {
			...
			printk("[FOCAL] enter always_on_panel mode\n");
			fts_ts_suspend(&fts_data->client->dev);
		}
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	vk_3v3=of_get_named_gpio_flags(np, "focaltech,power-gpio",0, NULL);
	...
	printk("bruce __dt the pdaata->irq_gpio:%d\n",pdata->irq_gpio);
	if (pdata->irq_gpio < 0)
	...
	qpnp_mpps_1 = of_get_named_gpio(np, "qpnp-mpp1", 0);
...
void focal3267_reset_ic(void)
{
	printk("[Focal]%s:reset IC start\n", __func__);
	...
	if (gpio_is_valid(private_ts->pdata->reset_gpio)) {
        ...
        //msleep(private_ts->soft_rst_dly);
	msleep(300);
	printk("[Focal]%s:reset IC end\n", __func__);
	return;
}
// add by leo --
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    status = gpio_request_one(vk_3v3,GPIOF_OUT_INIT_HIGH, "vk_3_3");
    if(status<0)
        printk("***request vk power gpio fail***");
	...
	{
		printk("[%s] ////////////////////		request_threaded_irq successed, chient->irq = %d", __func__, client->irq);
	}
	...
	__set_bit(APP_SWITCH, input_dev->keybit);
