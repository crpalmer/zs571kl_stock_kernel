...
{
	return TPReadReg(C6X36_CHANNEL_NUM, pPanelChannels);
}
...
{
	return TPReadReg(C6X36_KEY_NUM, pPanelKeys);
}
/************************************************************************
...
static int StartScan(void)
{
	unsigned char RegVal = 0x01;
    	...
    	ReCode = TPReadReg(C6208_SCAN_ADDR, &RegVal);
    	if(ReCode == ERROR_CODE_OK)
    	{
    		RegVal = 0x01;		//highest set 1 then start scan
    		ReCode = TPWriteReg(C6208_SCAN_ADDR, RegVal);
    		if(ReCode == ERROR_CODE_OK)
    		{
...
unsigned char ReadRawData(unsigned char Freq, unsigned char LineNum, int ByteNum, int *pRevBuffer)
{
    ...
    do{
        TPWriteReg(C6208_SCAN_ADDR, 0x01);
        msleep(50);
        /* need to get 0x00 on register 0x08
                ready for getting rawdata */
        TPReadReg( C6208_SCAN_ADDR, &I2C_wBuffer[0]);
...
static void AllocateMemory(void)
{
	...
	if(NULL == g_pStoreAllData)
		g_pStoreAllData = kmalloc(1024*80, GFP_KERNEL);
...
static unsigned char GetChannelNum(void)
{
	...
	if(ReCode == ERROR_CODE_OK)
	{
		g_ScreenSetParam.iKeyNum= 2;//rBuffer[0];
...
{	
	...
	for(i = 0; i < 3; i++)
	{
		ReCode = TPWriteReg(C6206_FACTORY_TEST_MODE, Proof_Normal);
		if(ERROR_CODE_OK == ReCode)
			ReCode = StartScan();
...
unsigned char FT6X36_TestItem_CbTest(unsigned char * bTestResult)
{
	...
	ReCode = TPReadReg( C6206_FACTORY_TEST_MODE, &chOldMode );
	...
	for(i = 0; i < 3; i++)
	{
		ReCode = TPWriteReg(C6206_FACTORY_TEST_MODE, Proof_Level0);
		if(ERROR_CODE_OK == ReCode)
			ReCode = StartScan();
		if(ERROR_CODE_OK == ReCode)break;
	}
	...
	else
	{
		...
		ReCode = TPWriteReg( C6206_FACTORY_TEST_MODE, chOldMode );
