...
/************************************************************************
* Name: TPReadReg(Same function name as FT_MultipleTest)
* Brief:  Read Register
* Input: RegAddr
* Output: RegData
* Return: Comm Code. Code = 0x00 is OK, else fail.
***********************************************************************/
int TPReadReg(unsigned char RegAddr, unsigned char *RegData)
{
	int iRet;
	/*
...
/************************************************************************
* Name: TPWriteReg(Same function name as FT_MultipleTest)
* Brief:  Write Register
* Input: RegAddr, RegData
* Output: null
* Return: Comm Code. Code = 0x00 is OK, else fail.
***********************************************************************/
int TPWriteReg(unsigned char RegAddr, unsigned char RegData)
{
	int iRet;
...
unsigned char EnterWork(void)
{
	...
	ReCode = TPReadReg(DEVIDE_MODE_ADDR, &RunState);
	if(ReCode == ERROR_CODE_OK)
	{
		...
		{
			ReCode = TPWriteReg(DEVIDE_MODE_ADDR, 0);
			if(ReCode == ERROR_CODE_OK)
			{
				ReCode = TPReadReg(DEVIDE_MODE_ADDR, &RunState);
				if(ReCode == ERROR_CODE_OK)
...
unsigned char EnterFactory(void)
{
	...
	if(ReCode == ERROR_CODE_OK)
	{
		...
		{
			ReCode = TPWriteReg(DEVIDE_MODE_ADDR, 0x40);
			if(ReCode == ERROR_CODE_OK)
			{
				ReCode = TPReadReg(DEVIDE_MODE_ADDR, &RunState);
				if(ReCode == ERROR_CODE_OK)
