...
extern void asus_smb1351_boost_enable(int enable, bool polling);
...
#define USBCON_TEMP_GPIO 132
#define FORCE_5V_IN_COS_FULL 1

/*us5587 registers*/
#define US5587_ADC_THERM_PMI8996	0x03  //pmi8996 charger
#define US5587_ADC_REG				0x04
#define US5587_ADC_THERM_SMB1351	0x05  //smb1351 charger
struct adc_i2c_dev {
...
static int therm_L1_temp = 45, therm_L2_temp = 48, therm_L3_temp = 55;
int therm_debug_time = -1;
...
static ssize_t charger_type_get(struct device *dev, struct device_attribute *devattr, char *buf)
{
        return sprintf(buf, "%s\n", input_type_str[g_smbchg_chip->asus_chg_type]);
}


static int smbchg_thermal_us5587_adc_temp(int);
static ssize_t pmi8996_thermal_get(struct device *dev, struct device_attribute *devattr, char *buf)
{
	int ret;
	printk("[BAT][CHG] %s start\n", __FUNCTION__);
	ret = smbchg_thermal_us5587_adc_temp(US5587_ADC_THERM_PMI8996);
    return sprintf(buf, "%d\n", ret);
}

static ssize_t smb1351_thermal_get(struct device *dev, struct device_attribute *devattr, char *buf)
{
	int ret;
	printk("[BAT][CHG] %s start\n", __FUNCTION__);
	ret = smbchg_thermal_us5587_adc_temp(US5587_ADC_THERM_SMB1351);
    return sprintf(buf, "%d\n", ret);
}
...
static ssize_t therm_debug_time_store(struct device *dev,
		struct device_attribute *devattr, const char *buf, size_t len)
{
	int tmp = 0;
	ssize_t ret;
	...
	if (tmp < 0) {
		pr_err("Error: echo therm debug time smaller than zero\n");
	} else {
		therm_debug_time = tmp;
	}
	return len;
}

static ssize_t therm_debug_time_show(struct device *dev,
		struct device_attribute *devattr, char *buf)
{
	return sprintf(buf, "%d\n", therm_debug_time);
}
...
static DEVICE_ATTR(therm_debug_time, S_IRUGO | S_IWUSR, therm_debug_time_show, therm_debug_time_store);
...
static struct attribute *qpnp_smbcharger_attributes[] = {
    ...
    &dev_attr_therm_debug_time.attr,
    NULL,
};
...
void asus_force_shipping_mode(void)
{
	...
	chip = g_smbchg_chip;
	smbchg_write(chip, &val, 0x12D0, 1);
	val = 0x0;
	smbchg_write(chip, &val, 0x1240, 1);
...
int asus_typec_DFP_type(enum typec_power_supply_type type, int voltage_mv, int current_ma){
	...
	if (type == TYPEC_UNKNOWN_CHARGER) {
		smbchg_dev->asus_typec_chg = type;
		pr_info("ignore unknown DFP type reported by CC\n");
		return 0;
	}
	if (smbchg_dev->asus_chg_type != HVDCP_ASUS_200K_2A)
		smbchg_dev->asus_typec_chg = type;
	else
		smbchg_dev->asus_typec_chg = TYPE_C_OTHERS;
	smbchg_dev->asus_pd_volt = voltage_mv;
	smbchg_dev->asus_cc_wa_flag = 1;

	if (current_ma == 1500 && voltage_mv == 5000)
		smbchg_dev->asus_pd_curr =1500;
	else if (current_ma >= 1000 && current_ma < 2000)
		smbchg_dev->asus_pd_curr = 1000;
	else if (current_ma >= 2000 && current_ma < 3000)
		smbchg_dev->asus_pd_curr = 2000;
	else if (current_ma >= 3000)
		smbchg_dev->asus_pd_curr = 3000;
	else
		smbchg_dev->asus_pd_curr = 0;
	return 0;
}
...
static int get_prop_batt_status(struct smbchg_chip *chip)
{
	...
	if ((get_prop_batt_capacity(chip) == 100)&&
		...
		if (!!g_charger_mode) {
			...
			pr_smb(PR_STATUS, "g_charger_mode = %d, disable device wakeup! \n", g_charger_mode);
		}
	}
out:
...
#define DEFAULT_BATT_FULL_CHG_CAPACITY	3300
...
static int smbchg_charging_en(struct smbchg_chip *chip, bool en)
{
	...
	if (g_ftm_mode) {
		if (asus_charger_limit_en && (batt_cap >= asus_charger_limit_percentage)){
			...
			pr_info("ftm mode. enable smb1351\n");
	...
	if (demo_app_status_flag && enableADF_check()) {
		if (batt_cap >= demo_app_charger_limit){
			...
			pr_info("demo app mode. enable smb1351\n");
			g_smb1351_NOT_charging_for_limit = false;
		}
	}
...
static void smbchg_hvdcp_det_work(struct work_struct *work)
{
	...
	if (is_hvdcp_present(chip)) {
		if (!chip->hvdcp3_supported &&
			...
			pr_info("smbchg_hvdcp_det_work: force_9v_hvdcp\n");
			if (rc)
				pr_err("could not force 9V HVDCP continuing rc=%d\n",
						rc);
		}
		pr_info("smbchg_hvdcp_det_work: smbchg_change_usb_supply_type\n");
...
void asus_allow_5v9v(void)
{
	struct smbchg_chip *chip = smbchg_dev;
	int rc;
	printk("allow 5v9v\n");
...
//UNDOME	if ((strcmp(androidboot_mode,"main")==0)&&(chip->boost_5v_vreg)&&
/*
	if ((chip->boost_5v_vreg)&&  //UNDOME
...
/*
  ...
  */
		switch (smbchg_dev->asus_pd_curr) {
			case 1000:
...
static int smbchg_type_c_dfp_setting(struct smbchg_chip *chip)
{
	switch (chip->asus_typec_chg) {
		case TYPEC_1_5A_CHARGER:
			chip->asus_chg_type = TYPEC_1P5A;
			pr_smb(PR_STATUS, "ASUS_CHG_TYPE: %s\n", input_type_str[chip->asus_chg_type]);
			return TYPE_C_1_5_A;
		case TYPEC_3A_CHARGER:
			chip->asus_chg_type = TYPEC_3P0A;
			pr_smb(PR_STATUS, "ASUS_CHG_TYPE: %s\n", input_type_str[chip->asus_chg_type]);
			return TYPE_C_3_A;
		case TYPEC_PD_5V_CHARGER:
		case TYPEC_PD_9V_CHARGER:
			chip->asus_chg_type = PD;
			pr_smb(PR_STATUS, "ASUS_CHG_TYPE: %s\n", input_type_str[chip->asus_chg_type]);
			return TYPE_C_PD;
		default:
...
void smbchg_type_c_det_work(struct work_struct *work)
{
	...
	switch (g_bc1p2_type) {
		...
		case USB_CDP:
			...
			break;
		case USB_DCP:
//UNDOME			if (strcmp(androidboot_mode,"recovery")==0) {
//UNDOME				pr_smb(PR_STATUS, "in recovery mode, start jeita and thermal immediately\n");
//UNDOME				queue_delayed_work(chip->smbchg_work_queue,
//UNDOME					&chip->thermal_policy_work,
//UNDOME					msecs_to_jiffies(0));
//UNDOME				queue_delayed_work(chip->smbchg_work_queue,
//UNDOME					&chip->check_status_work,
//UNDOME					msecs_to_jiffies(0));
//UNDOME			}
			break;
		default:
...
static int smbchg_thermal_us5587_adc_temp(int adc_reg)
{
	int i=0, adc_therm=0, table_size=0, temp_therm=0;
	/* select thermal policy level*/
	adc_therm = i2c_smbus_read_byte_data(us5587_i2c_dev->client, adc_reg);
	if (adc_therm < 0) {
...
void smbchg_thermal_policy_work(struct work_struct *work)
{
	...
	temp = smbchg_thermal_us5587_adc_temp(US5587_ADC_THERM_PMI8996);
...
out:
	if ((g_panel_off_flag)||(g_charger_mode)) {
	...
	if (therm_debug_time != -1) {
		next_time = therm_debug_time;
		queue_delayed_work(chip->smbchg_work_queue,
			&chip->thermal_policy_work,
			msecs_to_jiffies(next_time*1000));
	}
...
int smbchg_jeita_judge_state(int old_state, int batt_temp)
{
	...
	//10 <= batt_temp < 22
	} else if (batt_temp < 220) {
		result_state = JEITA_STATE_RANGE_03;
	//22 <= batt_temp < 52
	} else if (batt_temp < 520) {
...
static void smbchg_jeita_flow(struct smbchg_chip *chip)
{
	...
	pr_smb(PR_STATUS, "battery temperature: %d.%doC, battery voltage: %dmV, battery current: %dmA, battery soc: %d, battery msoc: %d(0x%02x), Vchg: 0x%02x\n",
		...
		batt_msoc,
		chg_volt_reg);
	ASUSEvtlog("[BAT][Ser]report Capacity==>%d,FCC:%dmA,BMS:%d",batt_soc,3200 ,batt_msoc*100/255);
	ASUSEvtlog(" V:%dmV,Cur:%dmA,Temp:%d.%doC",batt_volt/1000, batt_curr/1000, batt_temp/10, batt_temp%10);
	...
	if ((g_therm_unlock != 255)&&(g_usb_connector_event)) {
		pr_smb(PR_STATUS, "set dual charger suspend due to gpio 132 event\n");
		smbchg_suspend_enable(true);
		smb1351_set_suspend(true);
		goto finish;
	}
	...
	if (g_ftm_mode && asus_charger_limit_en) {
		if ((asus_charger_limit_percentage > batt_soc + 19 && asus_charger_limit_percentage >= 20) ||
	...
	switch (g_jeita_state) {
		...
		case JEITA_STATE_RANGE_05:
			...
			if (chg_volt_reg == FV_4P38 && batt_volt >= 4100000) {
				...
				fast_chg_reg_set = FCC_1300MA;
				recharge_enable = false;
				pr_smb(PR_STATUS, "500 <= temperature < 60, Vchg != 4.38V || Vbat < 4.1V\n");
			}
			break;
...
finish:
	...
	if ((g_charger_mode)&&(batt_soc == 100)) {
		/* switch to 5V HVDCP */
...
static void smbchg_dump_reg(struct smbchg_chip *chip, bool dump_all)
{
	...
	if (dump_all) {
		pr_smb(PR_STATUS, "================PMI8996 dump all regs start================\n");
		...
		pr_smb(PR_STATUS, "================PMI8996 dump all regs end==================\n");
        } else {
		pr_smb(PR_STATUS, "================PMI8996 dump regs start================\n");
		rc = smbchg_read(chip, &dump_reg, 0x10F2, 1);
		if (rc < 0)
			pr_err("Couldn't read 0x10F2 rc = %d\n", rc);
		else
			pr_smb(PR_STATUS, "FCC reg 0x10F2 = 0x%02x\n", dump_reg);
		...
		rc = smbchg_read(chip, &dump_reg, 0x130C, 1);
		if (rc < 0)
			pr_err("Couldn't read 0x130C rc = %d\n", rc);
		else
			pr_smb(PR_STATUS, "HVDCP STS reg 0x130C = 0x%02x\n", dump_reg);

		rc = smbchg_read(chip, &dump_reg, 0x13F2, 1);
		if (rc < 0)
			pr_err("Couldn't read 0x13F2 rc = %d\n", rc);
		else
			pr_smb(PR_STATUS, "IUSBIN reg 0x13F2 = 0x%02x\n", dump_reg);
		...
		pr_smb(PR_STATUS, "================PMI8996 dump regs end==================\n");
        }
...
static void smbchg_asus_adapter_detect_work(struct work_struct *work)
{
	...
	if (!g_switch_to_adc) {
		...
		pr_smb(PR_STATUS, "gpio_usbsw: 1 : %d\n",gpio_get_value(gpio_usbsw));
		...
		if (vadc < 0) {
			...
			if (vadc <= 0x3F) {
				...
				pr_smb(PR_STATUS, "gpio_adc_vh_en: 1 : %d\n",gpio_get_value(gpio_adc_vh_en));
				msleep(5);
				vadc = i2c_smbus_read_byte_data(us5587_i2c_dev->client, US5587_ADC_REG);
				if (vadc < 0) {
					pr_smb(PR_STATUS, "failed to read us5587 reg 0x%2x: %d\n", US5587_ADC_REG, vadc);
					...
					if (vadc > 0xD4) {
						...
						chip->asus_chg_type = DCP_OTHERS_1A;
						pr_smb(PR_STATUS, "ASUS_CHG_TYPE: %s\n", input_type_str[chip->asus_chg_type]);
						g_dual_charger_flag = DUALCHR_SINGLE;
					} else {
						//See Lookup Table(2A)
						if ((vadc >= 0x93) && (vadc <= 0xA8)) {
							if (g_hvdcp_flag == HVDCP_NONE) {
								chip->asus_chg_type = DCP_ASUS_750K_2A;
								ASUSEvtlog("[USB] set_chg_mode: ASUS AC");
								pr_smb(PR_STATUS, "ASUS_CHG_TYPE: %s\n", input_type_str[chip->asus_chg_type]);
								g_dual_charger_flag = DUALCHR_ASUS_2A;
							} else {
								chip->asus_chg_type = HVDCP_OTHERS_1A;
								pr_smb(PR_STATUS, "ASUS_CHG_TYPE: %s\n", input_type_str[chip->asus_chg_type]);
								g_dual_charger_flag = DUALCHR_SINGLE;
							}
						} else if ((vadc >= 0x3D) && (vadc <= 0x52)) {
							if (g_hvdcp_flag == HVDCP_QC_3_0) {
								chip->asus_chg_type = HVDCP_ASUS_200K_2A;
								ASUSEvtlog("[USB] set_chg_mode: ASUS AC");
								pr_smb(PR_STATUS, "ASUS_CHG_TYPE: %s\n", input_type_str[chip->asus_chg_type]);
								g_dual_charger_flag = DUALCHR_ASUS_2A;
							} else {
				...
				if (vadc > 0xBE) {
					...
					if (g_hvdcp_flag) {
						...
						chip->asus_chg_type = HVDCP_OTHERS_PB_1A;
						pr_smb(PR_STATUS, "ASUS_CHG_TYPE: %s\n", input_type_str[chip->asus_chg_type]);
						g_dual_charger_flag = DUALCHR_SINGLE;
					} else {
						chip->asus_chg_type = DCP_PB_2A;
						pr_smb(PR_STATUS, "ASUS_CHG_TYPE: %s\n", input_type_str[chip->asus_chg_type]);
						g_dual_charger_flag = DUALCHR_ASUS_2A;
					}
				} else {
					//Define Samsung adapter(1A)
					chip->asus_chg_type = OTHERS_1A;
					pr_smb(PR_STATUS, "ASUS_CHG_TYPE: %s\n", input_type_str[chip->asus_chg_type]);
					g_dual_charger_flag = DUALCHR_SINGLE;
				}
			}
...
//UNDOME			if (strcmp(androidboot_mode,"recovery")==0) {
//UNDOME				pr_smb(PR_STATUS, "in recovery mode, skip hvdcp detection\n");
//UNDOME				g_dual_charger_flag = DUALCHR_SINGLE;
//UNDOME				g_hvdcp_flag = HVDCP_NONE;
//UNDOME				queue_delayed_work(chip->smbchg_work_queue,
//UNDOME					&chip->type_c_det_work,
//UNDOME					msecs_to_jiffies(10*1000));
//UNDOME				break;
//UNDOME			}
			//enable QC 3.0
			hvdcp3_en_prop.intval = 1;
...
// this flag to indicate if otg boost ever requested by usb driver before smbcharger driver probed (ready)
// this case happened when booted with otg plugged
bool otg_ever_called = false;

int asus_otg_boost_enable(int enable)
{
	struct smbchg_chip *chip;
	int rc;
	if (g_smbchg_chip == NULL) {
	...
	if (enable) {
		...
		* gets detected as a valid input
		*/
		asus_smb1351_boost_enable(enable, chip->asus_is_otg);
		rc = smbchg_sec_masked_write(chip,
					chip->usb_chgpth_base + CHGPTH_CFG,
					HVDCP_EN_BIT, 0);
		if (rc < 0) {
		...
		asus_smb1351_boost_enable(enable, false);
...
/**
 * usb_connector_therm_handler() - called when the gpio 132 status changes.
...
static int smbchg_probe(struct spmi_device *spmi)
{
	...
	if (switch_dev_register(&charger_dev) < 0) {
		...
		g_usb_connector_event = gpio_get_value(USBCON_TEMP_GPIO);
		pr_smb(PR_STATUS, "check gpio 132 status first: %s\n", g_usb_connector_event ? "high" : "low");
		switch_set_state(&charger_dev, g_usb_connector_event);
	}
	...
	/* add gpio 132 interrupt func for usb thermal issue */
	rc = gpio_to_irq(USBCON_TEMP_GPIO);
	if (rc < 0) {
		pr_smb(PR_STATUS, "config gpio 132 as irq fail with rc: %d\n", rc);
	} else {
		...
		if (rc)
			pr_smb(PR_STATUS, "request gpio 132 irq handler failed with rc: %d\n", rc);
		else
			pr_smb(PR_STATUS, "request gpio 132 irq handler successed!!\n");
	}
	dev_info(chip->dev,
