...
//ASUS_BSP +++ "Load bdwlan30/qwlan30/utf30 bin files from /system/etc/firmware"
		if ( ( strncmp(buf->fw_id, "qwlan30", 7) == 0 ) || ( strncmp(buf->fw_id, "utf30", 5) == 0 ) ) {
			snprintf(path, PATH_MAX, "%s/%s", "/system/etc/firmware", buf->fw_id);
			dev_err(device, "[wlan] Try to load firmware : %s \n", path);
		}
		if ( strncmp(buf->fw_id, "bdwlan30", 8) == 0 ) {
			if ( g_ASUS_rfSKU == 1 ) {
				snprintf(path, PATH_MAX, "%s", "/system/etc/firmware/bdwlan30_NA.bin");
			} else {
				snprintf(path, PATH_MAX, "%s/%s", "/system/etc/firmware", buf->fw_id);
			}
			dev_err(device, "[wlan] Try to load firmware : %s \n", path);
