				  ...
				  unsigned gpio)
{
	...
	int pull;
	u32 ctl_reg;
	u32 val; // ASUS_BSP, show gpio level
	...
	val = readl(pctrl->regs + g->io_reg); // ASUS_BSP, show gpio level
...
static int wlan_irq_flag_rx = 0;
static int wlan_irq_flag_beacon = 0;

int wlan_irq_flag_function_rx(void)
{
    if( wlan_irq_flag_rx == 1 ) {
        wlan_irq_flag_rx = 0;
        return 1;
    }
...
EXPORT_SYMBOL(wlan_irq_flag_function_rx);

int wlan_irq_flag_function_beacon(void){
    if( wlan_irq_flag_beacon == 1 ){
        wlan_irq_flag_beacon = 0;
        wlan_irq_flag_rx = 0;
        return 1;
    }
...
EXPORT_SYMBOL(wlan_irq_flag_function_beacon);
//ASUS_BSP--- "for wlan wakeup trace"
