...
extern int rmnet_irq_flag_function_rx_168(void);//ASUS_BSP + Johnny [Qcom][PS][][ADD]Print first IP address log when IRQ 168
		...
		unsigned long data)
{
	...
	//ASUS_BSP +++ Johnny [Qcom][PS][][ADD]Print first IP address log when IRQ 168
	struct iphdr *ip4h;
	struct ipv6hdr *ip6h;
	...
	//ASUS_BSP +++ Johnny [Qcom][PS][][ADD]Print first IP address log when IRQ 168
	if (rmnet_irq_flag_function_rx_168() ==1 ){
		if (ip_version == 0x04){
			ip4h = (struct iphdr *) map_payload;
			pr_err_ratelimited("[data] IRQ168 First packet IP4 address src is %pI4",&ip4h->saddr);
		}else if (ip_version == 0x06){
			ip6h = (struct ipv6hdr *) map_payload;
			pr_err_ratelimited("[data] IRQ168 First packet IP6 address src is %pI6",&ip6h->saddr);
		}
	}
	//ASUS_BSP --- Johnny [Qcom][PS][][ADD]Print first IP address log when IRQ 168
