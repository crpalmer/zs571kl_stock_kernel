...
/* sysfs store for vibrator on/off setting */
static ssize_t qpnp_power_store(struct device *dev,
               struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (value == 1) {
		rc = gpio_direction_output(hap->power_gpio, 1);
		if (rc)
			printk("[Vibrator] power on failed !!!\n");
		else {
			printk("[Vibrator] %s : power on !!!\n", __func__);
		}
	}else if (value == 0) {
...
static struct device_attribute qpnp_hap_attrs[] = {
	...
	__ATTR(power_control, (S_IRUGO | S_IWUSR | S_IWGRP),
                        NULL,
                        qpnp_power_store),
...
/* set power_gpio off for haptics */
static int qpnp_hap_power_off(struct qpnp_hap *hap, int on)
{
	int rc = 0;

	if (g_ASUS_hwID >= 4) {
		if (on == 1) {
			if (gpio_get_value(hap->power_gpio) == 1) {
				rc = gpio_direction_output(hap->power_gpio, 0);
				if (rc)
					printk("[Vibrator] power off failed !!!\n");
				/*else {
					printk("[Vibrator] %s OK!!!\n", __func__);
				}*/
			}
		}
	}
...
static void qpnp_hap_td_enable(struct timed_output_dev *dev, int value)
{
	...
	if (value == 0) {
		if (hap->state == 0) {
			if (g_ASUS_hwID >= 4) {
				qpnp_hap_power_off(hap, 1);
			}
			mutex_unlock(&hap->lock);
			return;
		}
		else if (hap->state == 1 && hap->power_status == 1) {
			hap->power_status = 0;
		}
		hap->state = 0;
	} else {
...
static void qpnp_hap_worker(struct work_struct *work)
{
	...
	//printk("[Vibrator] enter %s , power_status=%d\n", __func__, hap->power_status);
...
static enum hrtimer_restart qpnp_hap_timer(struct hrtimer *timer)
{
	...
	//printk("[Vibrator] %s : power_status =0 \n", __func__);
	hap->power_status = 0;
	hap->state = 0;
	schedule_work(&hap->work);
...
static int qpnp_hap_config(struct qpnp_hap *hap)
{
	...
	/* add by angel for PR2 power*/
	if (gpio_is_valid(hap->power_gpio)) {
		rc = gpio_direction_output(hap->power_gpio, 0);
		if (rc) {
			printk("[%s] power_gpio direction fail !!!!!\n", __func__);
			return rc;
		}
		hap->power_status = 0;
	}
	printk("[%s] power_gpio = %d\n", __func__, gpio_get_value(hap->power_gpio));
	printk("[%s] power_status = %d\n", __func__, hap->power_status);
	/* add by angel for PR2 power*/
...
static int qpnp_hap_parse_dt(struct qpnp_hap *hap)
{
	...
	/* add by angel for PR2 power*/
	hap->power_gpio = of_get_named_gpio_flags(spmi->dev.of_node, "qcom,power-gpio", 0, &hap->power_gpio_flags);
	if (hap->power_gpio < 0)
		return hap->power_gpio;
	printk("%s: hap->power_gpio = %d\n", __func__, hap->power_gpio);
	/* add by angel for PR2 power*/
...
static int qpnp_haptic_probe(struct spmi_device *spmi)
{
	...
	printk("[Vibrator] probe start!!\n");
	if (g_ASUS_hwID >= 4) {
		printk("[Vibrator] PR2 Hardware!!\n");
	}
