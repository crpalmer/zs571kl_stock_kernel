...
static void dwc3_restart_usb_work(struct work_struct *w)
{
	...
	if (!timeout) {
		...
		dev_info(mdwc->dev, "ReStart:RT SUSP %d\n", atomic_read(&mdwc->dev->power.usage_count));
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_PRESENT:
		dev_dbg(mdwc->dev, "%s: notify xceiv event with val:%d\n",
							__func__, val->intval);
		printk("%s: notify xceiv event with val:%d\n",
							__func__, val->intval);
		/*
	...
	*/
	if (on) {
		...
		dbg_event(0xFF, "StrtHost gync",
			atomic_read(&mdwc->dev->power.usage_count));
		dev_info(mdwc->dev, "StrtHost gync %d\n", atomic_read(&mdwc->dev->power.usage_count));
		mdwc->hs_phy->flags |= PHY_HOST_MODE;
		...
		*/
		...
		if (ret) {
			...
			*/
			...
			dbg_event(0xFF, "pdeverr psync",
				atomic_read(&mdwc->dev->power.usage_count));
			dev_info(mdwc->dev, "pdeverr psync %d\n", atomic_read(&mdwc->dev->power.usage_count));
		...
		dbg_event(0xFF, "StrtHost psync",
			atomic_read(&mdwc->dev->power.usage_count));
		dev_info(mdwc->dev, "StrtHost psync %d\n", atomic_read(&mdwc->dev->power.usage_count));
		...
		*/
		...
		dbg_event(0xFF, "StopHost gsync",
			atomic_read(&mdwc->dev->power.usage_count));
		dev_info(mdwc->dev, "StopHost gsync %d\n", atomic_read(&mdwc->dev->power.usage_count));
		...
		dbg_event(0xFF, "StopHost psync",
			atomic_read(&mdwc->dev->power.usage_count));
		dev_info(mdwc->dev, "StopHost psync %d\n", atomic_read(&mdwc->dev->power.usage_count));
	}
...
static int dwc3_otg_start_peripheral(struct dwc3_msm *mdwc, int on)
{
	...
	dbg_event(0xFF, "StrtGdgt gsync",
		atomic_read(&mdwc->dev->power.usage_count));
	dev_info(mdwc->dev, "StrtGdgt gsync %d\n", atomic_read(&mdwc->dev->power.usage_count));
	...
	dbg_event(0xFF, "StopGdgt psync",
		atomic_read(&mdwc->dev->power.usage_count));
	dev_info(mdwc->dev, "StopGdgt psync %d\n", atomic_read(&mdwc->dev->power.usage_count));
...
static void dwc3_initialize(struct dwc3_msm *mdwc)
{
	...
	dbg_event(0xFF, "Initialized Start",
			atomic_read(&mdwc->dev->power.usage_count));
	dev_info(mdwc->dev, "Initialized Start %d\n", atomic_read(&mdwc->dev->power.usage_count));
	...
	case OTG_STATE_UNDEFINED:
		...
		if (!test_bit(B_SESS_VLD, &mdwc->inputs)) {
			...
			dbg_event(0xFF, "Undef NoUSB",
				atomic_read(&mdwc->dev->power.usage_count));
			dev_info(mdwc->dev, "Undef NoUSB %d\n", atomic_read(&mdwc->dev->power.usage_count));
			mdwc->otg_state = OTG_STATE_B_IDLE;
		}
		break;
	...
	case OTG_STATE_B_PERIPHERAL:
		if (!test_bit(B_SESS_VLD, &mdwc->inputs) ||
			...
			dbg_event(0xFF, "BPER psync",
				atomic_read(&mdwc->dev->power.usage_count));
			dev_info(mdwc->dev, "BPER psync %d\n", atomic_read(&mdwc->dev->power.usage_count));
			mdwc->chg_type = DWC3_INVALID_CHARGER;
			work = 1;
	...
	case OTG_STATE_B_SUSPEND:
		if (!test_bit(B_SESS_VLD, &mdwc->inputs)) {
			...
			dbg_event(0xFF, "SUSP gsync",
				atomic_read(&mdwc->dev->power.usage_count));
			dev_info(mdwc->dev, "SUSP gsync %d\n", atomic_read(&mdwc->dev->power.usage_count));
		}
		break;
