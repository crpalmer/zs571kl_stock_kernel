	...
	struct msm_vfe_axi_stream *stream_info, uint32_t pingpong_bit)
{
	...
	if (!temp_stream_info->undelivered_request_cnt) {
		...
		temp_stream_info->request_cnt[pingpong_bit]--;
...
static int msm_isp_update_stream_bandwidth(struct vfe_device *vfe_dev)
{
	...
	rc = msm_isp_update_bandwidth(ISP_VFE0 + vfe_dev->pdev->id,
		(total_bandwidth + vfe_dev->hw_info->min_ab)*200,
		(total_bandwidth + vfe_dev->hw_info->min_ib)*200);
			...
			int halt)
{
	...
	for (i = 0; i < stream_cfg_cmd->num_streams; i++) {
		...
		if (wait_for_complete_for_this_stream &&
			...
			frm_src = SRC_TO_INTF(stream_info->stream_src);
			/*We have only RDI stream..   *
			* Issue Reg update forcefully */
			ISP_DBG("%s: Issue reg update forcefully frame id %d\n",
				__func__,
				vfe_dev->axi_data.src_info[frm_src].frame_id);
	...
	uint32_t frame_id, uint32_t buf_index)
{
	...
	if (stream_info->undelivered_request_cnt == 1) {
		pingpong_status =
			vfe_dev->hw_info->vfe_ops.axi_ops.get_pingpong_status(
				vfe_dev);
		pingpong_bit = ((pingpong_status >> stream_info->wm[0]) & 0x1);
		if (stream_info->request_cnt[!pingpong_bit] > 0) {
	...
	if (stream_info->undelivered_request_cnt == 1) {
		...
		stream_info->request_cnt[0]++;
	} else if (stream_info->undelivered_request_cnt == 2) {
		stream_info->request_cnt[!pingpong_bit]++;
		rc = msm_isp_cfg_ping_pong_address(vfe_dev,
