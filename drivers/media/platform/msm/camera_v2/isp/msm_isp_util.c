	...
	uint32_t ping_pong_status)
{
  ...
  do {
	...
	if (queue_cmd->cmd_used) {
       ...
       vfe_dev->cascaded_irq_cnt =
               ((irq_status0 & (1 << 0)) +
               ((irq_status0 & (1 << 2)) >> 2) +
               ((irq_status0 & (1 << 25)) >> 25));

       if (vfe_dev->cascaded_irq_cnt > 1) {
               vfe_dev->is_cascaded_irq = 1;
               /*vfe_dev->cascaded_irq =
                       (irq_status0 & (1 << 0)) |
                       ((irq_status0 & (1 << 2)) >> 1) |
                       ((irq_status0 & (1 << 25)) >> 23);*/
               /*Mask one of the next pending irq*/
               irq_status0 = irq_status0 & ~(vfe_dev->next_irq);
               l_irq_status0 = vfe_dev->next_irq;
       } else {
       /* Assumption: Atleast once irq_status0 will not be cascaded *
         * with EPOCH or AXI*/
               vfe_dev->is_cascaded_irq = 0;
               //vfe_dev->cascaded_irq = 0;
               l_irq_status0 = irq_status0;
       }

       if ((l_irq_status0 & 0x2000005) == 0x1) {
               vfe_dev->next_irq = EPOCH;
       } else  if (((l_irq_status0 & 0x2000005) >> 2) == 0x1) {
               vfe_dev->next_irq = AXI_DONE;
       } else  if (((l_irq_status0 & 0x2000005) >> 25) == 0x1) {
               vfe_dev->next_irq = CAMIF_SOF;
       }

       queue_cmd->vfeInterruptStatus0 = l_irq_status0;
	queue_cmd->vfeInterruptStatus1 = irq_status1;
	queue_cmd->vfePingPongStatus = ping_pong_status;
	queue_cmd->is_cascaded_irq = vfe_dev->is_cascaded_irq;
	msm_isp_get_timestamp(&queue_cmd->ts, vfe_dev);
	ts = &queue_cmd->ts;
	trace_printk(" cascade: %d vfeid: %d irq_status0: 0x%x irq_status1: 0x%x frmid: %d ts: %ld \n",
		vfe_dev->is_cascaded_irq, vfe_dev->pdev->id, l_irq_status0, irq_status1,
		vfe_dev->axi_data.src_info[VFE_PIX_0].frame_id,
		ts->buf_time.tv_sec * 1000 + ts->buf_time.tv_usec / 1000);
	queue_cmd->cmd_used = 1;
	vfe_dev->taskletq_idx = (vfe_dev->taskletq_idx + 1) %
		MSM_VFE_TASKLETQ_SIZE;
	list_add_tail(&queue_cmd->list, &vfe_dev->tasklet_q);
   } while (vfe_dev->is_cascaded_irq);
...
irqreturn_t msm_isp_process_irq(int irq_num, void *data)
{
	...
	/*trace_printk("vfeid: %d frameid: %d irq_status0: 0x%x irq_status1: 0x%x \n",
