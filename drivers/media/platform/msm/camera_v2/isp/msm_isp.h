...
struct msm_vfe_axi_stream {
	...
	uint8_t request_cnt[MAX_REQUEST];
...
struct msm_vfe_tasklet_queue_cmd {
	...
	struct msm_isp_timestamp ts;
	uint8_t cmd_used;
	/*Cascaded IRQ's state machine*/
	uint8_t is_cascaded_irq;
	uint32_t cascaded_irq;
};
...
struct vfe_device {
	...
	/*Cascaded IRQ's state machine*/
	uint32_t is_cascaded_irq;
	uint32_t cascaded_irq_cnt;
	enum pix_irq_sm_t next_irq;
};
