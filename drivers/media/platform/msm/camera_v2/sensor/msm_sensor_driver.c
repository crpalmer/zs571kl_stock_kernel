...
/* eeprom memory size 131072 = 128KB */
...
static uint8_t *s_crc_ptr;
struct proc_dir_entry *Proc_File;
struct proc_dir_entry *Proc_CRC_File;
...
static int I2c_read_eeprom(void)
{
	...
	pr_info("%s:%d [eeprom]data_ptr Alloc memory success\n",__func__, __LINE__);
...
static ssize_t procfile_eeprom_read(struct file *file, char *buf, size_t count, loff_t *ppos)
{
    static int ret = 1;
	...
	*ppos += count;	/* increase offset */
	...
	pr_info("%s:%d [eeprom]dbg_dump_eeprom_read count:%zu \n", __func__, __LINE__,count);
	return count;
}

static struct file_operations cmd_file_ops = {
    .owner = THIS_MODULE,
    .read = procfile_eeprom_read,
};

int create_EEPROM_dump_proc_file(void)
{
	Proc_File = proc_create("EEPROM", S_IFREG | S_IRUGO, NULL, &cmd_file_ops);
	printk(KERN_INFO "/proc/EEPROM created\n");
    return 0;
}


static int I2c_read_eeprom_CRC(void)
{
	int rc = -1;
	uint32_t addr = 0x08;
	uint32_t i = 0;
	uint16_t i2c_slave_address;
	uint8_t *data_crc_ptr;
	struct msm_camera_i2c_client *sensor_i2c_client;

	data_crc_ptr = kzalloc(4, GFP_KERNEL);
	if (!data_crc_ptr) {
		pr_err("[eeprom]%s:%d data_crc_ptr Alloc memory Fail\n", __func__, __LINE__);
		return rc;
	}
	pr_info("%s:%d [eeprom]data_crc_ptr Alloc memory success\n", __func__, __LINE__);

#if 0
	/* Power up */
	pr_info("%s:%d [eeprom] Power up  :E\n", __func__, __LINE__);
	prc = msm_sensor_power_up(s_ctrl_depth);
	if (prc < 0) {
		pr_err("%s:%d [eeprom] power up failed\n", __func__, __LINE__);
		kfree(data_crc_ptr);
	...
	pr_info("[eeprom] i2c_read_seq  i=%d\n", i);
	rc = sensor_i2c_client->i2c_func_tbl->i2c_read_seq(
	sensor_i2c_client, addr, data_crc_ptr, 4);
	if (rc < 0){
		pr_err("%s:%d [eeprom] i2c read fail (eeprom_crc) rc =%d\n", __func__, __LINE__, rc);
		goto END;
	}
	memcpy(&s_crc_ptr[0], data_crc_ptr, 4);
...
static ssize_t procfile_eeprom_CRC_read(struct file *file, char *buf, size_t count, loff_t *ppos)
{

#if 0
	static int ret = 1;
	if (ret == 1)
		ret = I2c_read_eeprom_CRC();
	...
	if (copy_to_user(buf, s_crc_ptr + *ppos, count))
		return -EFAULT;
...
static struct file_operations eeprom_crc = {
	.owner = THIS_MODULE,
	.read = procfile_eeprom_CRC_read,
};

int create_EEPROM_CRC_dump_proc_file(void)
{
	Proc_CRC_File = proc_create("EEPROM_CRC", S_IFREG | S_IRUGO, NULL, &eeprom_crc);
	printk(KERN_INFO "/proc/EEPROM_CRC created\n");
	return 0;
}
...
CSID_TG:
	...
	*  sensor control structure */
	...
	/*
	 * Depth Camera => camera_id = 3
	 * Dump eeprom of depth camera
	 */
	if (slave_info->camera_id == 3){

		/* ========== procfs ======= */
		s_ptr = kzalloc(MAX_eeprom, GFP_KERNEL);
		if (!s_ptr){
			pr_err("[eeprom]%s:%d eeprom Mem Alloc Fail\n", __func__, __LINE__);
			rc = -1;
			goto camera_power_down;
		}

		s_crc_ptr = kzalloc(4, GFP_KERNEL);
		if (!s_crc_ptr){
			pr_err("[eeprom_crc]%s:%d eeprom_crc Mem Alloc Fail\n", __func__, __LINE__);
			rc = -1;
			goto free_s_ptr;
		}

		pr_info("%s:%d [eeprom]Mem Alloc success\n", __func__, __LINE__);
		s_ctrl_depth = s_ctrl;
		create_EEPROM_CRC_dump_proc_file();
		create_EEPROM_dump_proc_file();

		pr_info("%s:%d [eeprom_crc]I2c_read_eeprom_CRC :E\n", __func__, __LINE__);
		rc = I2c_read_eeprom_CRC();
		if (rc < 0)
			pr_err("[eeprom_crc]%s:%d I2c_read_eeprom_CRC Fail\n", __func__, __LINE__);

		/* ========== debugfs ======= */
		//msm_debugfs_eeprom_init(s_ctrl_depth);
	}
