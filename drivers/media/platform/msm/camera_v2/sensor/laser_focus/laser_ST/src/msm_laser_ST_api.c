...
{
	int32_t rc = 0;
	CDBG("Enter g_ASUS_hwID %d \n",g_ASUS_hwID);

	/*Siva is only use hepta laser, ignore this function*/
	CDBG("Siva is hepta laser, ignore this function\n");
	return 0;

	mutex_init(&vl6180x_mutex);
	pr_info(" laser_ST_platform_probe Enter\n");
	...
	if (rc == 0) {
		...
		pr_err("laser platform_driver_register failed , laser_ST_platform_probe \n");
