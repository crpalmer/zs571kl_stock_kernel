	...
	*power_setting_size = count;
	...
	for (i = 0; i < count; i++) {
		...
		case SENSOR_GPIO:
			...
			else if (!strcmp(seq_name, "sensor_gpio_imgstart"))
				ps[i].seq_val = SENSOR_GPIO_IMGSTART;
			else if (!strcmp(seq_name, "sensor_gpio_cam3mode"))
				ps[i].seq_val = SENSOR_GPIO_CAM3MODE;
			else if (!strcmp(seq_name, "sensor_gpio_dldomode"))
				ps[i].seq_val = SENSOR_GPIO_DLDOMODE;
			else if (!strcmp(seq_name, "sensor_gpio_vcselmode"))
				ps[i].seq_val = SENSOR_GPIO_VCSELMODE;
			else
				rc = -EILSEQ;
			break;
		case SENSOR_CLK:
	...
	uint16_t gpio_array_size)
{
	...
	rc = of_property_read_u32(of_node, "qcom,gpio-imgstart", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-imgstart failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-imgstart invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_IMGSTART] =
			gpio_array[val];
		gconf->gpio_num_info->valid[SENSOR_GPIO_IMGSTART] = 1;
		CDBG("%s qcom,gpio-imgstart %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_IMGSTART]);
	} else {
		rc = 0;
	}
	rc = of_property_read_u32(of_node, "qcom,gpio-cam3mode", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-cam3mode failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-cam3mode invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_CAM3MODE] =
			gpio_array[val];
		gconf->gpio_num_info->valid[SENSOR_GPIO_CAM3MODE] = 1;
		CDBG("%s qcom,gpio-cam3mode %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_CAM3MODE]);
	} else {
		rc = 0;
	}
	rc = of_property_read_u32(of_node, "qcom,gpio-dldomode", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-dldomode failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-dldomode invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_DLDOMODE] =
			gpio_array[val];
		gconf->gpio_num_info->valid[SENSOR_GPIO_DLDOMODE] = 1;
		CDBG("%s qcom,gpio-dldomode %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_DLDOMODE]);
	} else {
		rc = 0;
	}
	rc = of_property_read_u32(of_node, "qcom,gpio-vcselmode", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-vcselmode failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-vcselmode invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_VCSELMODE] =
			gpio_array[val];
		gconf->gpio_num_info->valid[SENSOR_GPIO_VCSELMODE] = 1;
		CDBG("%s qcom,gpio-vcselmode %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_VCSELMODE]);
	} else {
		rc = 0;
	}
