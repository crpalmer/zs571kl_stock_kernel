...
int32_t msm_camera_cci_i2c_read_adv(
	struct msm_camera_i2c_client *client,
	struct msm_camera_i2c_reg_setting *read_setting,
	uint16_t *data)
{
	int32_t rc = -EFAULT;
	struct msm_camera_cci_ctrl cci_ctrl;
	unsigned char buf[MSM_CAMERA_I2C_WORD_ADDR + MSM_CAMERA_I2C_WORD_DATA];

	if (read_setting->size == 0) {
		pr_err("%s@%d: invalid size= %d\n", __func__, __LINE__, read_setting->size);
		return rc;
	}
	if (read_setting->size > 1) {
		pr_err("%s@%d: unsupported size= %d\n", __func__, __LINE__, read_setting->size);
		return rc;
	}
	if (read_setting->addr_type < MSM_CAMERA_I2C_BYTE_ADDR &&
	read_setting->addr_type > MSM_CAMERA_I2C_WORD_ADDR) {
		pr_err("%s@%d: invalid addr_type= %d\n", __func__, __LINE__, read_setting->addr_type);
		return rc;
	}
	if (read_setting->data_type < MSM_CAMERA_I2C_BYTE_DATA &&
	read_setting->data_type > MSM_CAMERA_I2C_WORD_DATA) {
		pr_err("%s@%d: invalid data_type= %d\n", __func__, __LINE__, read_setting->data_type);
		return rc;
	}

	cci_ctrl.cmd = MSM_CCI_I2C_READ;
	cci_ctrl.cci_info = client->cci_client;
	cci_ctrl.cfg.cci_i2c_read_cfg.addr = read_setting->reg_setting[0].reg_addr;
	cci_ctrl.cfg.cci_i2c_read_cfg.addr_type = read_setting->addr_type;
	cci_ctrl.cfg.cci_i2c_read_cfg.data = buf;
	cci_ctrl.cfg.cci_i2c_read_cfg.num_byte = read_setting->data_type;
	...
	S_I2C_DBG("%s addr = 0x%x data: 0x%x\n", __func__, read_setting->reg_setting[0].reg_addr, *data);
	return rc;
}
