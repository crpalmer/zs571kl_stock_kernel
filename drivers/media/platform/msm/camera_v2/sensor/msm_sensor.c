...
int msm_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
  ...
  const char *sensor_name;
	...
	if (s_ctrl->power_count > 1){
		s_ctrl->power_count = s_ctrl->power_count - 1;
		return 0;
	} else {
		s_ctrl->power_count = 0;
	}
	...
	void __user *argp)
{
	...
	int32_t rc = 0;
	int32_t i = 0;
	static uint8_t is_i2c_read = false;
	const char *sensor_name = s_ctrl->sensordata->sensor_name;
	if (!strcmp(sensor_name, "imx318") || !strcmp(sensor_name, "ov8856"))
		mutex_lock(s_ctrl->msm_cci0_mutex);
	else
	...
	case CFG_SLAVE_WRITE_I2C_ARRAY: {
		...
		struct msm_camera_i2c_reg_setting conf_array;
		struct msm_camera_i2c_reg_array *reg_setting = NULL;
		struct msm_camera_i2c_reg_array *user_setting = NULL;
		...
		conf_array.delay = conf_array32.delay;
		conf_array.size = conf_array32.size;
		user_setting = compat_ptr(conf_array32.reg_setting);
#if 1
		read_slave_addr = conf_array.slave_id;
#else
		/**
		 * @note Keep slave address intact
		 */
		read_slave_addr = s_ctrl->sensor_i2c_client->cci_client->sid;
#endif
		orig_slave_addr = s_ctrl->sensor_i2c_client->cci_client->sid;
		s_ctrl->sensor_i2c_client->cci_client->sid = read_slave_addr;
		...
		for (i=0;i<conf_array.size;i++) {
			CDBG("%s@%d: reg[%d]= (addr= 0x%04x, value= 0x%04x)\n", __func__, __LINE__, i, conf_array.reg_setting[i].reg_addr, conf_array.reg_setting[i].reg_data);
		}
	...
	case CFG_READ_I2C_ADV:
	case CFG_SLAVE_READ_I2C_ADV: {
		struct msm_camera_i2c_reg_setting32 reg_setting32;
		struct msm_camera_i2c_reg_setting reg_setting;
		struct msm_camera_i2c_reg_array *reg_array = NULL;
		uint16_t local_data = 0;
		uint16_t default_id, read_id; // I2C slave IDs (7-bit)

		int16_t slave_id_set(struct msm_sensor_ctrl_t *s_ctrl, uint16_t new_id, uint16_t *old_id) {
			...
			if (NULL == s_ctrl->sensor_i2c_client) {
				return(-EINVAL);
			}
			sensor_i2c_client = s_ctrl->sensor_i2c_client;
			if (sensor_i2c_client->cci_client) {
				cci_client = sensor_i2c_client->cci_client;
				if (old_id) {
					*old_id = cci_client->sid;
					CDBG("%s: new I2C slave ID= 0x%02x (old= 0x%02x)", __func__, new_id, *old_id);
				} else {
					CDBG("%s: new I2C slave ID= 0x%02x", __func__, new_id);
				}
				cci_client->sid = new_id;
			} else if (sensor_i2c_client->client) {
				client = sensor_i2c_client->client;
				if (old_id) {
					*old_id = client->addr;
				}
				client->addr = new_id;
		...
		if (copy_from_user(&reg_setting32,
			(void *)compat_ptr(cdata->cfg.setting),
			sizeof(struct msm_camera_i2c_reg_setting32))) {
			pr_err("%s:%d failed\n", __func__, __LINE__);
			rc = -EFAULT;
		...
		reg_setting.addr_type = reg_setting32.addr_type;
		reg_setting.data_type = reg_setting32.data_type;
		reg_setting.delay = reg_setting32.delay;
		reg_setting.size = reg_setting32.size;
		reg_setting.slave_id = reg_setting32.slave_id;
		reg_setting.reg_setting = compat_ptr(reg_setting32.reg_setting);
		...
		reg_array = kzalloc(reg_setting.size *
			(sizeof(struct msm_camera_i2c_reg_array)), GFP_KERNEL);
		if (!reg_array) {
			pr_err("%s:%d failed\n", __func__, __LINE__);
			rc = -ENOMEM;
		...
		if (copy_from_user(reg_array, (void *)reg_setting.reg_setting,
			reg_setting.size *
			sizeof(struct msm_camera_i2c_reg_array))) {
			pr_err("%s:%d failed\n", __func__, __LINE__);
			kfree(reg_array);
		...
		if (CFG_SLAVE_READ_I2C_ADV == cdata->cfgtype) {
			CDBG("%s: CFG_SLAVE_READ_I2C_ADV\n", __func__);
			read_id = reg_setting.slave_id >> 1;
			rc = slave_id_set(s_ctrl, read_id, &default_id);
			if (rc) {
				pr_err("%s: failed to set slave_id= 0x%02x(8bit)= 0x%02x(7bit)", __func__, read_id, read_id>>1);
			}
		} else {
			CDBG("%s: CFG_READ_I2C_ADV\n", __func__);
		}
		rc = msm_camera_cci_i2c_read_adv(s_ctrl->sensor_i2c_client,
			&reg_setting, &local_data);
		if (CFG_SLAVE_READ_I2C_ADV == cdata->cfgtype) {
			slave_id_set(s_ctrl, default_id, NULL);
		}
		if (rc < 0) {
		...
		reg_setting.reg_setting[0].reg_data = local_data;
		CDBG("%s: Local data read = 0x%x\n", __func__, local_data);
	...
	case CFG_READ_I2C: {
		...
		CDBG("I2C_READ %s:CFG_READ_I2C: read_config.reg_addr 0x%x is_i2c_read %d\n",
			__func__, read_config.reg_addr, is_i2c_read);
		...
		CDBG("%s: Local data read = 0x%x\n", __func__, read_config.data);

		if (copy_to_user((void *)compat_ptr(cdata->cfg.setting),
			(void *)&read_config, sizeof(struct msm_camera_i2c_read_config))) {
...
#if 1 	//VJ
		CDBG("Local data read = 0x%x\n", read_config.data);
		pr_err("Local data read = 0x%x\n", read_config.data);
	...
	case CFG_GPIO_OP: {
		struct msm_cam_gpio_operation gpio_op;

		if (copy_from_user(&gpio_op,
			(void *)compat_ptr(cdata->cfg.setting),
		...
		gpio_set_value_cansleep( gpio_op.address, (gpio_op.value) ?
			GPIOF_OUT_INIT_HIGH : GPIOF_OUT_INIT_LOW);
		pr_err("%s gpio %d value %d is_i2c_read %d\n", __func__,
			gpio_op.address, gpio_op.value, is_i2c_read);
		break;
	}
	default:
		rc = -EFAULT;
...
int msm_sensor_config(struct msm_sensor_ctrl_t *s_ctrl, void __user *argp)
{
	...
	case CFG_GPIO_OP: {
		...
		pr_err("GPIO %s gpio %d value %d\n", __func__,
			gpio_op.address, gpio_op.value);
	}
