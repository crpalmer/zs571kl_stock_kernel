	...
	struct msm_camera_i2c_reg_setting_array *settings)
{
  ...
  rc = flash_ctrl->flash_i2c_client.i2c_func_tbl->i2c_write_table(
	...
	CDBG("%s:%d [%d] addr_type %d data_type %d delay %d setting.size %d\n",
			__func__, __LINE__, i, settings->addr_type, settings->data_type, settings->delay, settings->size);
	for (i = 0; i < conf_array.size; i++){
			CDBG("%s:%d [%d] reg_addr 0x%x reg_data 0x%x\n",
			__func__, __LINE__, i, settings->reg_setting_a[i].reg_addr, settings->reg_setting_a[i].reg_data);
	}
	CDBG("%s:%d i2c write rc = %d\n",
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	if (flash_ctrl->flash_device_type == MSM_CAMERA_PLATFORM_DEVICE) {
		...
		cci_client->sid = flash_init_info->slave_addr;
  ...
  rc = msm_camera_fill_vreg_params(flash_ctrl->power_info.cam_vreg,
		flash_ctrl->power_info.num_vreg, flash_ctrl->power_info.power_setting,
		flash_ctrl->power_info.power_setting_size);
	if (rc < 0) {
		pr_err("%s:%d msm_camera_fill_vreg_params failed rc %d\n",
			__func__, __LINE__, rc);
		//return rc;
	}
	...
	struct msm_flash_cfg_data_t *flash_data, int32_t bulb_ctrl)
{
	...
	switch (bulb_ctrl) {
			...
			case 1:
					current_data.reg_data = 0x20;
					break;
			case 2:
					current_data.reg_data = 0x02;
					break;
			case 3:
					current_data.reg_data = 0x22;
					break;
			case 4:
					current_data.reg_data = 0x10;
					break;
			case 5:
					current_data.reg_data = 0x01;
					break;
			case 6:
					current_data.reg_data = 0x11;
					break;
			default:
	...
	struct msm_flash_cfg_data_t *flash_data)
{
  struct msm_camera_i2c_reg_setting conf_array;
	unsigned i = 0;
	int32_t rc = -EINVAL;
	struct msm_camera_i2c_reg_array flash_current_data[2];
  ...
  switch (flash_cmd) {
    case CFG_FLASH_LOW:
      torch_current_data.reg_addr = 0x03;
      torch_current_data.reg_data = ((flash_data->flash_current[1] & 0xf) << 4) | ((flash_data->flash_current[0] & 0xf));
      torch_current_data.delay = 0x00;
      pr_err("%s:%d reg_addr: 0x%x, reg_data: 0x%x\n", __func__, __LINE__,
    ...
    case CFG_FLASH_HIGH:
      for(i = 0; i < 2; i++) {
        ...
        flash_current_data[i].reg_data = ((flash_data->flash_current[i] & 0x1f));
...
struct msm_flash_ctrl_t *gBSP_flash_ctrl = 0;
...
static int32_t msm_flash_platform_probe(struct platform_device *pdev)
{
	...
	CDBG("probe success\n");
		...
		if (msm_debugfs_flash_init(flash_ctrl) < 0)
			pr_err("initial camera flash debugfs fail!");
	#endif

	gBSP_flash_ctrl = flash_ctrl;
	create_proc_file();
	return rc;
}
  ...
  switch (flash_mode) {
    case 1: // FLASH
      ...
      if (flash_current <= FLASH_MODULE_DEFAULT_CURRENT) {
        ...
        pr_err("%s original flash_current %d, reg flash_current %d\n", __func__, flash_current, rc);
      }
    ...
    case 2: //TORCH
      ...
      pr_err("%s original torch_current %d, reg torch_current %d\n", __func__, flash_current, rc);
      break;
  }
  return rc;
}
...
static int32_t msm_flash_custom_init(struct msm_flash_ctrl_t *flash_ctrl)
{
  ...
  flash_ctrl->flash_i2c_client.i2c_func_tbl->i2c_util(&flash_ctrl->flash_i2c_client, MSM_CCI_INIT);
  ...
  if (flash_ctrl->power_info.gpio_conf->gpio_num_info->
	...
	for (i = 0; i < conf_array.size; i++){
		...
		if(rc < 0)
		  ...
		  pr_err("%s:%d i2c_read data[%d] = %d, rc = %d\n", __func__, __LINE__, i, reg_data, rc);
	}
  ...
  struct msm_flash_ctrl_t *flash_ctrl)
{
  struct msm_camera_i2c_reg_setting conf_array;
	int32_t rc = -EINVAL;
	struct msm_camera_i2c_reg_array torch_current_data;
  torch_current_data.reg_addr = 0x04;
  torch_current_data.reg_data = 0x10;
  torch_current_data.delay = 0x00;
  pr_err("%s:%d reg_addr: 0x%x, reg_data: 0x%x\n", __func__, __LINE__,
  torch_current_data.reg_addr, torch_current_data.reg_data);
  conf_array.reg_setting = &torch_current_data;
...
static ssize_t msm_flash_brightness_proc_write(struct file *filp, const char __user *buf, size_t count, loff_t *ppos)
{
    ...
    int rc = 0, len;
    struct msm_flash_ctrl_t *flash_ctrl = gBSP_flash_ctrl;
    struct msm_flash_cfg_data_t flash_data;
    ...
    *ppos=len;
    ...
    pr_debug("[AsusFlashBrightness]flash brightness value= %d now_flash_brightness_value = %d\n", set_val,now_flash_brightness_value);

    if (last_flash_brightness_value == now_flash_brightness_value
        ||  now_flash_brightness_value > 99) {
        pr_info("[AsusFlashBrightness] input value does not change or out of range, do nothing\n");
        return len;
    }
    ...
    /*Set input current mA +++*/
    {
        ...
        val1 = set_val - FLASH_MODULE_STEP_TORCH_CURRENT * (set_val / FLASH_MODULE_STEP_TORCH_CURRENT);
        val2 = FLASH_MODULE_STEP_TORCH_CURRENT * (set_val / FLASH_MODULE_STEP_TORCH_CURRENT + 1) - set_val;
        if (ISSMALLER(val1, val2))
            set_val = FLASH_MODULE_STEP_TORCH_CURRENT * (set_val / FLASH_MODULE_STEP_TORCH_CURRENT);
        else
          set_val = FLASH_MODULE_STEP_TORCH_CURRENT * (set_val / FLASH_MODULE_STEP_TORCH_CURRENT + 1);
    }
    /*Set input current mA ---*/
    if (set_val > MAX_FLASHLIGHT_CURRENT) {
        pr_debug("%s:%d [AsusLedInterface] set_val > MAX_FLASHLIGHT_CURRENT %d, use MAX_FLASHLIGHT_CURRENT",
        __func__, __LINE__, MAX_FLASHLIGHT_CURRENT);
        set_val = MAX_FLASHLIGHT_CURRENT;
    }

    if (0 < set_val) {
        flash_data.flash_current[0] = 0;
        flash_data.flash_current[1] = translate_current_to_reg_data(set_val, 2);
        flash_data.cfg_type = CFG_FLASH_LOW;
        pr_debug("%s:%d [AsusLedInterface] flash_current reg_data %d",
        __func__, __LINE__, flash_data.flash_current[1]);
